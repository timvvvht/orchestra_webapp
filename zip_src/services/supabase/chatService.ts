...
import { buildSessionMetadata, SessionMetadataInput } from '@/utils/buildSessionMetadata';
...
const parseDateSafelyToEpochMs = (dateString: string | null | undefined): number => {
    if (!dateString) {
        return NaN;
    }
 ...
 */
export const updateSessionTitle = async (sessionId: string, displayTitle: string): Promise<void> => {
    try {
        const currentUserId = await getCurrentUserId();
        const { error } = await supabase.from('chat_sessions').update({ display_title: displayTitle }).eq('id', sessionId).eq('user_id', currentUserId);

        if (error) throw error;
        console.log(`[chatService] Updated session title for ${sessionId}: "${displayTitle}"`);
    } catch (error) {
        console.error(`[chatService] Error updating session title for ${sessionId}:`, error);
        throw error;
    }
};
 ...
 */
const mapToSessionMeta = (dbSession: SupabaseChatSessionRow & { metadata_avatar_url?: string; metadata_agent_name?: string }): SessionMeta => {
    // Use safe date parsing to handle invalid timestamps
    const createdAt = parseDateSafelyToEpochMs(dbSession.created_at);
    const lastUpdated = parseDateSafelyToEpochMs(dbSession.last_message_at || dbSession.created_at);
 ...
 */
const mapToChatSession = (dbSession: SupabaseChatSessionRow, messages: ChatMessage[] = []): ChatSession => {
    ...
    return {
        ...
        metadata: metadata,
        initialAgentSnapshot: (dbSession.initial_agent_snapshot as Record<string, any>) || null,
...
const convertJsonToRichContent = (content: Json): RichContentPart[] => {
    ...
    console.log('[chatService] Converting content array with', content.length, 'items');

    // Map each item in the array to a valid RichContentPart
    return content.map((item, index) => {
        console.log(`[chatService] Converting item ${index}:`, JSON.stringify(item));
        ...
        if ('type' in item) {
            ...
            if (type === 'tool_result' || type === 'tool_output') {
                ...
                const toolResultPart = {
                    type: 'tool_result',
                    tool_use_id: String(item.tool_use_id_for_output || item.tool_use_id || ''),
                    content: rawContent, // Keep as original type instead of stringifying
                    is_error: Boolean(item.status === 'error' || item.is_error || false)
                } as ToolResultPart;
                console.log('[chatService] Converted tool_result (preserving object):', toolResultPart);
                return toolResultPart;
            }
        }

        // Fallback: convert to text part
        console.warn('[chatService] convertJsonToRichContent: Unrecognized content part, converting to text:', {
            type: item.type,
 ...
 */
const mapToChatMessage = (dbMessage: SupabaseChatMessageRow & { message_metadata?: any }): ChatMessage => {
    ...
    if (isNaN(parsedPrimaryTimestamp)) {
        console.warn(`[ChatService] mapToChatMessage: Message ${dbMessage.id} missing 'timestamp'. Defaulting to epoch start (0).`);
        msgCreatedAt = 0;
    } else {
...
const mapToInsertChatSession = (
    sessionData: Partial<
        Pick<
            ChatSession,
            'name' | 'avatar' | 'agent_config_id' | 'specialty' | 'model' | 'tools' | 'systemPrompt' | 'temperature' | 'user_id' | 'initialAgentSnapshot'
        >
    >
    ...
    if (sessionData.systemPrompt) metadata.system_prompt = sessionData.systemPrompt;
...
const mapToInsertChatMessage = (
    messageData: Omit<ChatMessage, 'id' | 'timestamp' | 'isStreaming' | 'thinking' | 'reasoning' | 'delivered' | 'read' | 'debugSourceEvent'>
): SupabaseInsertChatMessage => {
    const supabaseContent: SupabaseRichContentPart[] = messageData.content.map(part => {
        if (part.type === 'text') {
            return { type: 'text', text: part.text } as SupabaseRichContentPart;
        }
        if (part.type === 'tool_use') {
            return { type: 'tool_use', tool_use_id: part.id, tool_name: part.name, tool_input: part.input } as SupabaseRichContentPart;
        }
        ...
        console.warn('[chatService] Unknown RichContentPart type during mapToInsertChatMessage:', part);
        return { type: 'text', text: '[Unsupported part type in store message]' } as SupabaseRichContentPart;
    });
    ...
    return {
        ...
        tool_call_id: messageData.content.find(p => p.type === 'tool_use')?.id || null,
        responding_to_tool_use_id: messageData.content.find(p => p.type === 'tool_result')?.tool_use_id || null,
        metadata: Object.keys(metadata).length > 0 ? metadata : null
    };
};
...
export const createChatSession = async (
    ...
    try {
        ...
        console.log(`[chatService] Creating chat session: ${insertData.name} (${insertData.id})`);
        ...
        console.log(`[chatService] Created chat session: ${data.name} (${data.id}) with complete metadata`);
 ...
 */
export const getChatSession = async (sessionId: string, options: { messageLimit?: number } = {}): Promise<ChatSession | null> => {
    try {
        const currentUserId = await getCurrentUserId();
        ...
        const messages = messagesResult.data ? messagesResult.data.reverse().map(mapToChatMessage) : [];

        console.log(`[chatService] Fetched chat session for ID ${sessionId} (user: ${currentUserId}) with ${messages.length} messages`);
        return mapToChatSession(sessionResult.data, messages);
    } catch (error) {
        console.error(`[chatService] Error fetching chat session for ID ${sessionId}:`, error);
        throw error;
    }
};
...
export const getAllChatSessions = async (): Promise<ChatSession[]> => {
    try {
        ...
        console.log(`[chatService] Fetched ${data.length} chat sessions for user: ${currentUserId}`);
...
export const getAllChatSessionMetas = async (): Promise<SessionMeta[]> => {
    try {
        ...
        console.log(`[chatService] Fetched ${data.length} chat session metas for user: ${currentUserId}`);
 ...
 */
export const getChatSessionsBatch = async (sessionIds: string[], options: { messageLimit?: number } = {}): Promise<(ChatSession | null)[]> => {
    ...
    try {
        ...
        console.log(`[chatService] Batch fetched ${results.filter(r => r !== null).length}/${sessionIds.length} sessions`);
        return results;
    } catch (error) {
...
export const updateChatSession = async (
    sessionId: string,
    updates: Partial<
        Pick<ChatSession, 'name' | 'avatar' | 'specialty' | 'model' | 'tools' | 'systemPrompt' | 'temperature' | 'lastUpdated' | 'initialAgentSnapshot'>
    >
): Promise<ChatSession> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        ...
        if (!currentSession) throw new Error(`Chat session with ID ${sessionId} not found or not owned by current user`);

        // Prepare the update data
        const updateData: SupabaseUpdateChatSession = {};

        if (updates.name) updateData.name = updates.name;
        if (updates.lastUpdated) updateData.last_message_at = new Date(updates.lastUpdated).toISOString();
        if (updates.initialAgentSnapshot) updateData.initial_agent_snapshot = updates.initialAgentSnapshot;

        // Merge metadata
        const currentMetadata = (currentSession.metadata as Record<string, any>) || {};
        ...
        if (updates.systemPrompt !== undefined) newMetadata.system_prompt = updates.systemPrompt;
        if (updates.temperature !== undefined) newMetadata.temperature = updates.temperature;
        ...
        const { data, error } = await supabase.from('chat_sessions').update(updateData).eq('id', sessionId).eq('user_id', currentUserId).select().single();

        if (error) throw error;
        if (!data) throw new Error(`Failed to update chat session with ID ${sessionId}`);
        ...
        console.log(`[chatService] Updated chat session for ID ${sessionId}:`, data);
        return mapToChatSession(data, messages);
    } catch (error) {
        console.error(`[chatService] Error updating chat session for ID ${sessionId}:`, error);
        throw error;
    }
};
...
export const deleteChatSession = async (sessionId: string): Promise<void> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        const { error } = await supabase.from('chat_sessions').delete().eq('id', sessionId).eq('user_id', currentUserId);

        if (error) throw error;

        console.log(`[chatService] Deleted chat session with ID: ${sessionId} (user: ${currentUserId})`);
    } catch (error) {
        console.error(`[chatService] Error deleting chat session with ID ${sessionId}:`, error);
        throw error;
    }
};
 ...
 */
export const saveChatMessage = async (messageData: Omit<ChatMessage, 'id' | 'timestamp'>): Promise<ChatMessage> => {
    try {
        ...
        await supabase.from('chat_sessions').update({ last_message_at: data.timestamp }).eq('id', messageData.sessionId);
 ...
 */
export const saveChatMessageBatch = async (messagesData: Omit<ChatMessage, 'id' | 'timestamp'>[]): Promise<ChatMessage[]> => {
    ...
    try {
        ...
        if (error) throw error;
        if (!data || data.length === 0) throw new Error('Failed to save chat messages batch');
        ...
        await supabase.from('chat_sessions').update({ last_message_at: latestMessage.timestamp }).eq('id', latestMessage.session_id);
 ...
 */
export const getChatMessages = async (sessionId: string, options: GetChatMessagesOptions = {}): Promise<ChatMessage[]> => {
    console.log(`[ChatService] getChatMessages called for ${sessionId} with options:`, options);
    try {
        let query = supabase
            .from('chat_messages')
            .select('id, session_id, role, content, timestamp, model_used, tool_call_id, responding_to_tool_use_id, message_metadata:metadata')
            .eq('session_id', sessionId);

        const effectiveOrderBy = options.orderBy || { column: 'timestamp', ascending: false }; // Default to newest first
        ...
        return data.map(dbMsg => mapToChatMessage(dbMsg as SupabaseChatMessageRow & { message_metadata?: any }));
    } catch (error) {
...
export const getAllChatMessages = async (sessionId: string): Promise<ChatMessage[]> => {
    console.log(`[ChatService] getAllChatMessages called for session: ${sessionId}`);
    const currentUserId = getOrSetAnonymousUserId();

    // First verify the user owns this session for security
    const { data: session, error: sessionError } = await supabase.from('chat_sessions').select('user_id').eq('id', sessionId).single();

    if (sessionError || !session || session.user_id !== currentUserId) {
        console.error('[ChatService] Session not found or access denied for session ' + sessionId + ' (user: ' + currentUserId + ')');
        throw new Error('Session not found or access denied');
    }

    const selectColumns = 'id, session_id, role, content, timestamp, model_used, tool_call_id, responding_to_tool_use_id';
    ...
    if (error) {
        console.error('[ChatService] Error fetching all chat messages for session ' + sessionId + ' (user: ' + currentUserId + '):', error);
        throw error;
    }

    if (!data) {
        console.warn('[ChatService] No data returned when fetching all messages for session ' + sessionId + ' (user: ' + currentUserId + ')');
        return [];
    }

    const mappedMessages: ChatMessage[] = data.map(dbMsg => mapToChatMessage(dbMsg as SupabaseChatMessageRow & { message_metadata?: any }));
    console.log(`[ChatService] getAllChatMessages: Fetched and mapped ${mappedMessages.length} messages for session ${sessionId}.`);
    // log tool result parts
    mappedMessages.forEach(msg => {
        console.log(`[ChatService] getAllChatMessages: Message ${msg.id} has ${msg.content.length} parts.`);
        msg.content.forEach(part => {
            if (part.type === 'tool_result') {
                // log FULL message
                console.log(`[ChatService] getAllChatMessages: Tool result part ${part.tool_use_id} has content:`, part.content);
            }
        });
    });
 ...
 */
export const forkConversation = async (parentSessionId: string, forkRequest: ForkRequest): Promise<ChatSession> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        ...
        const parentMetadata = (parentSession.metadata as Record<string, any>) || {};
        const completeMetadata = buildSessionMetadata(parentMetadata);

        console.log(`[chatService] Creating fork with complete metadata inherited from parent`);
        ...
        console.log(`[chatService] Created forked session ${newSession.id} from parent ${parentSessionId} at message ${forkRequest.messageId}`);

        // Return the new session (with empty messages array initially)
        return mapToChatSession(newSession, []);
    } catch (error) {
        console.error(`[chatService] Error forking conversation from ${parentSessionId}:`, error);
        throw error;
    }
};
 ...
 */
export const getForkedConversationMessages = async (sessionId: string): Promise<ForkedMessage[]> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        ...
        const { data: rawData, error } = await (supabase.rpc as any)('get_forked_conversation', { target_session_id: sessionId });

        if (error) throw error;
        if (!rawData) return [];

        // Map the response to ForkedMessage objects
        const messages: ForkedMessage[] = (rawData as GetForkedConversationResponse[]).map((row: GetForkedConversationResponse) => ({
            id: row.message_id,
        ...
        console.log(`[chatService] Fetched ${messages.length} messages for forked conversation ${sessionId}`);
        return messages;
    } catch (error) {
        console.error(`[chatService] Error fetching forked conversation messages for ${sessionId}:`, error);
        throw error;
    }
};
 ...
 */
export const getConversationForks = async (sessionId: string): Promise<ForkInfo[]> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        ...
        const { data, error } = await supabase.rpc('get_conversation_forks', { session_id: sessionId });
        ...
        console.log(`[chatService] Found ${forks.length} forks for conversation ${sessionId}`);
        return forks;
    } catch (error) {
        console.error(`[chatService] Error fetching conversation forks for ${sessionId}:`, error);
        throw error;
    }
};
 ...
 */
export const getForkAncestry = async (sessionId: string): Promise<ConversationAncestry[]> => {
    try {
        const currentUserId = getOrSetAnonymousUserId();
        ...
        const { data, error } = await supabase.rpc('get_fork_ancestry', { session_id: sessionId });

        if (error) throw error;
        if (!data) return [];

        // Map the response to ConversationAncestry objects
        const ancestry: ConversationAncestry[] = data.map((row: GetForkAncestryResponse) => ({
            id: row.ancestor_id,
        ...
        console.log(`[chatService] Found ${ancestry.length} ancestors for session ${sessionId}`);
        return ancestry;
    } catch (error) {
        console.error(`[chatService] Error fetching fork ancestry for ${sessionId}:`, error);
        throw error;
    }
};
