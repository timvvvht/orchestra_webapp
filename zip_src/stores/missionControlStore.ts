...
import { Plan, PlanProgress } from '@/types/plans';
import { analyzePlanProgressDetailed } from '@/utils/planProgress';
import { ParsedPlanResult } from '@/utils/plan';
import { baseDirFromCwd } from '@/utils/pathHelpers';
import { GitStatusCounts } from '@/utils/gitHelpers';
...
export const useMissionControlStore = create<MissionControlState>((set, get) => {
    ...
    return {
        ...
        setPlans: plans =>
            set(() => {
                ...
                const progressMap = plans.reduce((acc, plan) => {
                    ...
                    console.log(`[plan] Calculated progress for session ${plan.session_id}:`, progress);
                    return acc;
        ...
        setPlanRefetchCallback: callback => set(state => (state.planRefetchCallback === callback ? state : { planRefetchCallback: callback })), 
        ...
        setSessionRefetchCallback: callback => set(state => (state.sessionRefetchCallback === callback ? state : { sessionRefetchCallback: callback })), 
        ...
        refetchSinglePlan: async (sessionId) => {
            ...
            try {
                const { data: plan, error } = await supabase
                    ...
                    .maybeSingle();
                if (error) { console.warn('[plan] Single fetch error:', error); return; }
                if (!plan) { console.warn('[plan] No plan for session', sessionId); return; }
                const progress = analyzePlanProgressDetailed(plan.markdown);
                set(state => ({
                    plans: { ...state.plans, [sessionId]: plan },
                    planProgress: { ...state.planProgress, [sessionId]: progress }
                }));
                console.log(`[plan] Single plan refetch complete for session ${sessionId}`);
            } catch (e) {
        ...
        patchPlanFromToolResult: (sessionId, parsed) =>
            set(state => {
                console.log(`[plan] Patching plan for session ${sessionId} with:`, parsed);

                const currentPlan = state.plans[sessionId];
                if (!currentPlan) {
                    console.warn(`[plan] No existing plan found for session ${sessionId}, skipping patch`);
                    return state;
                }
                ...
                const updatedPlan: Plan = {
                    ...
                    version: parsed.version || currentPlan.version
                };

                // Recalculate progress for this plan
                const updatedProgress = analyzePlanProgressDetailed(updatedMarkdown);

                console.log('[UI-SSE][planPatched]', 'session=', sessionId, 'newProgress=', updatedProgress);
        ...
        ensureInProcessingOrder: (sessionId: string) =>
            set(state => {
                ...
                console.log('[UI] Session added to processing order:', sessionId, `(position: ${updated.length})`);
        ...
        // Ambient indicator actions
        setLastCheckpointSaved: (timestamp: string | null) => set({ lastCheckpointSaved: timestamp }),
