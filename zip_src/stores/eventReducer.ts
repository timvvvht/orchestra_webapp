...
function isDuplicate(event: CanonicalEvent): boolean {
    ...
    if (event.toolUseId && (event.kind === 'tool_call' || event.kind === 'tool_result')) {
...
function upsertEvent(state: EventState, event: CanonicalEvent) {
    ...
    if (!existingEvent) {
        ...
        if (!sessionEvents.includes(event.id)) {
            const sessionInsertIndex = findInsertionIndexSession(sessionEvents, event, state);
...
function removeEvent(state: EventState, eventId: string) {
    ...
    // Clean up tool correlation
    if (event.toolUseId) {
        const toolLink = state.toolIx.get(event.toolUseId);
        if (toolLink) {
            ...
            if (!toolLink.call && !toolLink.result) {
                state.toolIx.delete(event.toolUseId);
            }
        }
    }
}
 ...
 */
function findInsertionIndex(state: EventState, newEvent: CanonicalEvent): number {
 ...
 */
function findInsertionIndexSession(sessionEvents: string[], newEvent: CanonicalEvent, state: EventState): number {
    const newTimestamp = new Date(newEvent.createdAt).getTime();
...
function updateToolIndex(state: EventState, event: CanonicalEvent) {
    ...
    state.toolIx.set(event.toolUseId, toolLink);
}
...
export const eventSelectors = {
    getAllEvents: (state: EventState): CanonicalEvent[] => state.order.map(id => state.byId.get(id)!).filter(Boolean),

    getEventById: (state: EventState, id: string): CanonicalEvent | undefined => state.byId.get(id),

    // 🔥 NEW SESSION SELECTORS
    getEventsForSession: (state: EventState, sessionId: string): CanonicalEvent[] => {
        const sessionEventIds = state.bySession.get(sessionId) || [];
        return sessionEventIds.map(id => state.byId.get(id)!).filter(Boolean);
    },

    getSessionIds: (state: EventState): string[] => Array.from(state.bySession.keys()),

    getSessionEventCount: (state: EventState, sessionId: string): number => state.bySession.get(sessionId)?.length ?? 0,

    getToolPair: (state: EventState, toolUseId: string): { call?: CanonicalEvent; result?: CanonicalEvent } => {
