...
export async function hydrateSession(sessionId: string): Promise<void> {
    ...
    try {
        ...
        while (hasMore) {
            ...
            console.log(`[HistoryBridge] Fetched ${rawRows?.length || 0} raw messages from Supabase`);

            // Hydration debug probe - DB stage
            if (rawRows && rawRows.length > 0) {
                import('@/utils/hydroDebug').then(({ pushHydro }) => {
                    pushHydro('DB', { sessionId, rowCount: rawRows.length, firstRow: rawRows[0] });
                });
            }
            ...
            console.log(`[HistoryBridge] Converting ${rawRows.length} messages to canonical events...`);
            let canonicalEvents;
            try {
                canonicalEvents = mapBatch(rawRows as any[]);
                console.log(`[HistoryBridge] Converted to ${canonicalEvents.length} canonical events`);
                if (canonicalEvents.length > 0) {
                    console.log(`[HistoryBridge] First canonical event:`, canonicalEvents[0]);
                }
            } catch (error) {
            ...
            console.log(`[HistoryBridge] Processed page: ${rawRows.length} messages → ${canonicalEvents.length} events (total: ${totalEvents})`);
        ...
        console.log(`[HistoryBridge] ✅ Hydrated session ${sessionId} with ${totalEvents} total events`);
    } catch (error) {
...
async function hydrateCheckpoints(sessionId: string): Promise<void> {
    try {
        console.log(`[HistoryBridge] Loading checkpoints for session: ${sessionId}`);
        ...
        const checkpointsWithHash = checkpoints.filter(checkpoint => {
            if (!checkpoint.commit_hash) {
                console.log(`[HistoryBridge] ✂️ Skipping checkpoint without commit hash: ${checkpoint.phase} - ${new Date(checkpoint.created_at).toISOString()}`);
                return false;
            }
            return true;
        });
        ...
        const sortedCheckpoints = [...checkpointsWithHash].sort((a, b) => 
        ...
        console.log(`[HistoryBridge] Processing ${sortedCheckpoints.length} checkpoints with commit hashes:`);
        sortedCheckpoints.forEach((cp, i) => {
            console.log(`  ${i}: ${cp.phase} - ${cp.commit_hash.slice(0, 8)} - ${new Date(cp.created_at).toISOString()}`);
        });
        
        for (const checkpoint of sortedCheckpoints) {
            console.log(`[HistoryBridge] Processing ${checkpoint.phase} checkpoint ${checkpoint.commit_hash.slice(0, 8)}, mostRecentCommitHash: ${mostRecentCommitHash?.slice(0, 8) || 'none'}`);
            
            if (checkpoint.commit_hash === mostRecentCommitHash) {
                console.log(`[HistoryBridge] ✂️ Skipping redundant ${checkpoint.phase} checkpoint ${checkpoint.commit_hash.slice(0, 8)} (same as most recent)`);
                continue;
            }
            
            console.log(`[HistoryBridge] ✅ Keeping ${checkpoint.phase} checkpoint ${checkpoint.commit_hash.slice(0, 8)}`);
            filteredCheckpoints.push(checkpoint);
            mostRecentCommitHash = checkpoint.commit_hash;
            console.log(`[HistoryBridge] 📝 Updated mostRecentCommitHash to ${mostRecentCommitHash.slice(0, 8)}`);
        }
        
        console.log(`[HistoryBridge] Filtered ${checkpoints.length} checkpoints to ${filteredCheckpoints.length} non-redundant ones with commit hashes`);
        ...
        console.log(`[HistoryBridge] ✅ Hydrated ${checkpointEvents.length} checkpoint events`);
    } catch (error) {
...
export async function hydrateSessions(sessionIds: string[]): Promise<void> {
    const isEnabled = import.meta.env.VITE_CANONICAL_STORE === '1' || import.meta.env.VITE_CANONICAL_STORE === 'true';
    if (!isEnabled) {
        console.log('[HistoryBridge] Skipped bulk hydration (feature flag disabled)');
        return;
    }

    console.log(`[HistoryBridge] Bulk hydrating ${sessionIds.length} sessions`);

    const results = await Promise.allSettled(sessionIds.map(sessionId => hydrateSession(sessionId)));

    const successful = results.filter(r => r.status === 'fulfilled').length;
    const failed = results.filter(r => r.status === 'rejected').length;

    console.log(`[HistoryBridge] Bulk hydration complete: ${successful} successful, ${failed} failed`);
