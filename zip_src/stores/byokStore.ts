...
import type { APIKeyProviderResponse } from '@/services/acs';
  ...
  persist(
    (set, get) => ({
      ...
      fetchStoredKeyProviders: async () => {
        ...
        try {
          ...
          if (session?.access_token && !acsClient.isAuthenticated()) {
            console.log('🔄 [byokStore] Syncing Supabase token to ACS client...');
            acsClient.setAuthToken(session.access_token);
          }
        } catch (error) {
          console.warn('⚠️ [byokStore] Failed to check Supabase session:', error);
        }

        // First check if we have a token at all (race condition protection)
        const currentToken = acsClient.getAuthToken();
        if (!currentToken || currentToken === '') {
          console.warn('⚠️ [byokStore] Auth token not ready, aborting API call');
          set({ keyProvidersError: 'Auth not ready', isLoadingKeyProviders: false });
          return;
        }

        // Check if client is authenticated
        if (!acsClient.isAuthenticated()) {
          console.warn('⚠️ [byokStore] ACS client not authenticated, aborting API call');
          set({ keyProvidersError: 'Authentication required', isLoadingKeyProviders: false });
          return;
        }

        // Double-check that we have a valid token length
        if (currentToken.length < 10) {
          console.warn('⚠️ [byokStore] ACS client token appears invalid:', { tokenLength: currentToken.length });
          set({ keyProvidersError: 'Invalid authentication token', isLoadingKeyProviders: false });
          return;
        }
        ...
        console.log('🔍 [byokStore] ACS client state:', {
          ...
          authTokenStart: acsClient.getAuthToken()?.substring(0, 50) + '...' || 'undefined',
        });
        ...
        try {
          ...
          if (response && Array.isArray(response)) {
            ...
            set({ storedKeyProviders: providers, isLoadingKeyProviders: false });
          } else {
      ...
      removeStoredKeyProvider: (provider: string) => {
        set({ storedKeyProviders: get().storedKeyProviders.filter(p => p !== provider) });
      }
    }),
    {
