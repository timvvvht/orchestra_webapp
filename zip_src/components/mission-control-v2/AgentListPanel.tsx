...
import { Loader2, Clock, Sparkles, ArrowRight, Save, GitBranch, Archive } from 'lucide-react';
...
const AgentListPanel: React.FC = () => {
  ...
  const unreadIdle = groupedSessions.idle.filter(agent => isSessionUnread(agent.id));
  const readIdle = groupedSessions.idle.filter(agent => !isSessionUnread(agent.id));
        ...
        <div className="min-h-[400px]">
                ...
                {viewMode === 'archived' ? (
                  ...
                  <>
                    ...
                    <div className="flex justify-center gap-3 mb-10">
                      {['Exploring', 'Coding', 'Testing'].map((label, i) => (
                        <motion.div
                          key={label}
                    ...
                    <div className="mt-10 flex items-center justify-center gap-6">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-white/50">Auto-checkpoint</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <GitBranch className="w-4 h-4 text-blue-400/60" />
                        <span className="text-xs text-white/50">Isolated branches</span>
                      </div>
                    </div>
                  </>
                )}
          ...
          {groupedSessions.processing.length > 0 && viewMode === 'active' && (
            <AgentGroup
              ...
              icon={<Loader2 className="w-3.5 h-3.5 text-white/50 animate-spin" />}
          ...
          {viewMode === 'archived' && (
            <AgentGroup
              ...
              count={groupedSessions.processing.length + unreadIdle.length + readIdle.length}
              isCollapsed={false}
              groupKey="drafts" // Use drafts as placeholder since archived doesn't collapse
            >
              <div className="px-6">
                {[...groupedSessions.processing, ...unreadIdle, ...readIdle].map(agent => (
                  <AgentCard
                    key={agent.id}
                    agent={agent}
                    group="idle"
