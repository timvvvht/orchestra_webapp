...
const TextPartDisplay: React.FC<TextPartDisplayProps> = ({ part }) => {
    const [mermaidDiagrams, setMermaidDiagrams] = useState<{id: string, content: string}[]>([]);
    const [processedContent, setProcessedContent] = useState<string>('');
    const [copiedId, setCopiedId] = useState<string | null>(null);
    
    // Extract Mermaid diagrams and detect file paths
    useEffect(() => {
        const extractMermaidDiagrams = () => {
            const diagrams: {id: string, content: string}[] = [];
            let content = part.text;
            
            // Filter out content within <instructions> tags
            content = content.replace(/<instructions>[\s\S]*?<\/instructions>/gi, '');
        ...
        const { content, diagrams } = extractMermaidDiagrams();
        
        // File path detection disabled
        const contentWithFileLinks = content;
        
        setMermaidDiagrams(diagrams);
        setProcessedContent(contentWithFileLinks);
    }, [part.text]);
    ...
    // Renderers designed for mental clarity and emotional calm
    const renderers = {
    ...
    return (
        <div className={cn(
            ...
            "text-[0.9375rem] leading-[1.6] tracking-[-0.011em]", // 15px optimized for reading
            ...
            {/* Mermaid diagrams - treated as first-class content */}
