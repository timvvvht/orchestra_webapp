...
export const ChatScrollAnchor: React.FC<ChatScrollAnchorProps> = ({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        console.log(`⚓ [ScrollAnchor] Intersection changed: ${entry.isIntersecting}, boundingRect: ${JSON.stringify(entry.boundingClientRect)}`);
  ...
  useEffect(() => {
    if (shouldAutoScroll) {
      console.log(`⚓ [ScrollAnchor] Auto-scrolling triggered, isAtBottom: ${isAtBottom}`);
      scrollToBottom();
    }
