...
const ChatMainCanonicalLegacyComponent: React.FC<ChatMainCanonicalLegacyProps> = ({
  ...
  const shouldAutoScroll = useMemo(() => {
    ...
    console.log(`🔄 [AutoScroll] Session: ${sessionId}, shouldAutoScroll: ${result}, isAtBottom: ${isAtBottom}, anchorVisible: ${anchorVisible}, messageCount: ${messages.length}`);
    return result;
  ...
  const handleScroll = useCallback((event: any) => {
    ...
    console.log(`📜 [Scroll] Session: ${sessionId}, atBottom: ${atBottom}, scrollTop: ${scrollTop}, scrollHeight: ${scrollHeight}, clientHeight: ${clientHeight}`);

    setIsAtBottom(atBottom);
    setHasScrolledUp(scrolledUpSignificantly);

    // Lazy render: Load more messages when scrolling near top
    if (scrollTop < 150 && renderLimit < mergedMessages.length && !loadingOlderRef.current) {
      loadingOlderRef.current = true;
      setRenderLimit(prev => Math.min(prev + RENDER_BATCH_INCREMENT, mergedMessages.length));
      setTimeout(() => { loadingOlderRef.current = false; }, 200); // debounce guard
    }
  ...
  return (
    <div ref={mainContainerRef} className={getContextClasses()}>
      ...
      <HydrationDebugOverlay open={hydrationDebugOverlayOpen} onClose={() => setHydrationDebugOverlayOpen(false)} />
