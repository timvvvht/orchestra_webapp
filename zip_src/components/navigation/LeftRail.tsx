...
import { Home, Bot, FileText, Settings, Code, LogOut, Activity, Zap, Wand2, GitBranch, GitCommit } from 'lucide-react';
...
export const LeftRail: React.FC = () => {
  ...
  const handleVaultClick = (e: React.MouseEvent) => {
    ...
    if (!user) {
      console.log('[LeftRail] Vault access requires authentication, showing auth modal');
      setShowModal(true);
      return;
    }
  ...
  const handleNavigation = (to: string, e: React.MouseEvent) => {
    ...
    if (!user) {
      e.preventDefault();
      console.log(`[LeftRail] ${to} requires authentication, showing auth modal`);
      setShowModal(true);
      return;
    }
