...
export const ApprovalPanel: React.FC<ApprovalPanelProps> = ({
  ...
  const loadPendingApprovals = useCallback(async () => {
    console.log(`🔧 [ApprovalPanel] Loading pending approvals for session: ${sessionId}`);
    try {
      ...
      if (result.success) {
        ...
        setError(null);
        console.log(`🔧 [ApprovalPanel] Set ${result.data?.length || 0} pending approvals for session ${sessionId}`);
      } else {
        console.error(`🔧 [ApprovalPanel] Failed to load approvals:`, result.message);
