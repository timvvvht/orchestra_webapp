...
const MainLayoutContent: React.FC<MainLayoutContentProps> = ({ children, title }) => {
  const { 
    showFilePanel, 
    sidePanelWidth, 
    setSidePanelWidth,
  ...
  const [teamStatus, setTeamStatus] = useState<'auto' | 'paused' | 'off'>('off');
  const [viewMode, setViewMode] = useState<'tasks' | 'email' | 'social'>('tasks');
  ...
  useEffect(() => {
    ...
    if (!resizeHandle || !filePanel) {
      console.log('Resize handle or file panel ref not available', { resizeHandle, filePanel });
      return;
    }
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      document.documentElement.style.setProperty('--file-panel-width', `${limitedWidth}px`);
    };
    ...
    <div className="flex h-full w-full overflow-hidden min-h-0">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          {/* Content area */}
          <div className={`flex-1 ${isMissionControlV2 ? 'overflow-hidden min-h-0' : 'overflow-auto'} bg-surface-0`}>
            <div className={`h-full ${isMissionControlV2 ? 'overflow-hidden min-h-0' : 'overflow-auto'}`}>
              {error ? (
