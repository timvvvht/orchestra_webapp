...
import { sendChatMessage, SendChatMessageOptions } from '@/utils/sendChatMessage';
import { sendChatWithHooks } from '@/utils/sendChatWithHooks';

export async function checkRepositoryState(codePath: string): Promise<{ isDirty: boolean; isGit: boolean }> {
    console.log(`[taskOrchestration][checkRepositoryState] Checking repo state for: ${codePath}`);
    
    const isGit = await isGitRepo(codePath);
    console.log(`[taskOrchestration][checkRepositoryState] isGit: ${isGit}`);
    
    if (!isGit) {
        console.log(`[taskOrchestration][checkRepositoryState] Not a git repo, returning clean state`);
    ...
    console.log(`[taskOrchestration][checkRepositoryState] Final result:`, result);
    return result;
}

export async function createTaskSession(sessionName: string, agentConfigId: string): Promise<string> {
    // This function directly wraps the existing fast ID creation logic.
    return createSessionFast(sessionName, agentConfigId);
}

// This function assumes the repository is clean. The check is now separate.
export async function prepareTaskWorkspace(sessionId: string, projectRoot: string): Promise<string> {
    // Returns the path to the new worktree's CWD.
    const worktreeCwd = await invoke<string>('create_worktree', { sessionId, projectRoot });
    return worktreeCwd;
}

export async function updateTaskCwd(sessionId: string, cwd: string): Promise<void> {
    const acs = getDefaultACSClient();
    await acs.sessions.updateSession(sessionId, { agent_cwd: cwd });
}

export async function setSessionBaseDir(sessionId: string, baseDir: string): Promise<void> {
    const acs = getDefaultACSClient();
...
export async function sendInitialTaskMessage(options: SendChatMessageOptions & { cwd?: string }): Promise<{ success: boolean; error?: string }> {
