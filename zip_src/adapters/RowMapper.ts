...
import { CanonicalEvent, MessageEvent, ToolCallEvent, ToolResultEvent, RichContentNode } from '@/types/events';
...
export class RowMapper {
            ...
            if (contentArray.length > 0) {
                for (const contentPart of contentArray) {
                    if (contentPart.type === 'text') {
                        ...
                        const toolUseId = contentPart.id || contentPart.tool_use_id || row.tool_call_id || this.generateToolUseId();
                        const toolName = contentPart.name || contentPart.tool_name || 'unknown';
                    ...
                    } else if (contentPart.type === 'tool_output' || contentPart.type === 'tool_result') {
                        // Create tool result event
                        const toolUseId =
                            contentPart.tool_use_id || contentPart.tool_use_id_for_output || row.responding_to_tool_use_id || this.generateToolUseId();
            ...
            return [
                {
                    ...
                    content: [
                        {
                            type: 'text',
                            text: `Error parsing message: ${error instanceof Error ? error.message : 'Unknown error'}`
                        }
                    ]
                }
            ];
    ...
    private static mapContentPart(contentPart: any): RichContentNode {
        ...
        if (contentPart.type === 'tool_use' && contentPart.input) {
            ...
            textContent = typeof contentPart.text === 'object' ? JSON.stringify(contentPart.text, null, 2) : String(contentPart.text);
        }
    ...
    private static safeParseLegacyResult(result: any): any {
        ...
        if (Array.isArray(result)) {
            ...
            if (result.length > 0 && result[0] && typeof result[0] === 'object' && 'text' in result[0]) {
    ...
    private static validateToolCallResultPairs(events: CanonicalEvent[]): void {
        ...
        const toolResults = events.filter(e => e.kind === 'tool_result') as ToolResultEvent[];
    ...
    static canMap(row: any): row is ExtendedSupabaseDbChatMessage {
        return row && typeof row === 'object' && typeof row.id === 'string' && typeof row.timestamp === 'string' && typeof row.role === 'string';
    }
}
