 ...
 */
function normalizeToolResult(toolResultData: any): {
  toolCallId: string;
  result: any;
  ok: boolean;
  error?: any;
  ...
  let toolCallId =
    toolResultData?.result?.tool_use_id || // common OpenAI-style
    toolResultData?.result?.tool_call_id || // alternate
    toolResultData?.tool_call_id || // top-level
    toolResultData?.toolCallId || // camelCase
    toolResultData?.tool_use_id || // sometimes sent at top-level
    toolResultData?.tool?.id || // defensive: nested tool object
    toolResultData?.call?.id || // defensive: nested call object
    toolResultData?.tool_call?.id || // defensive: nested tool_call object
    toolResultData?.toolUse?.id || // defensive
    ""; // fallback

  // FINAL FALLBACK: mirror tool_call structure on tool_result events
  // If backend repeats the call structure, we can read data.tool_call.id
  if (!toolCallId) {
    const nestedCallId =
      toolResultData?.tool_call?.id ||
      toolResultData?.toolCall?.id ||
      toolResultData?.tool?.id ||
      undefined;
    if (nestedCallId) toolCallId = nestedCallId;
  }

  // Defensive cleanup: coerce to string and trim
  if (toolCallId != null) {
  ...
  if (!resultContent) {
    ...
    if (Array.isArray(contentArray) && contentArray[0]?.text) {
      ...
      console.log(
        "🔧 [normalizeToolResult] Flattened content array to text:",
  ...
  if (
    ...
    (resultContent.startsWith("{") || resultContent.startsWith("["))
  ) {
    try {
      const parsed = JSON.parse(resultContent);
      console.log(
        "🔧 [normalizeToolResult] Successfully parsed JSON:",
        typeof parsed
      );
      resultContent = parsed;
    } catch (error) {
      console.log(
        "🔧 [normalizeToolResult] JSON parse failed, keeping as string:",
        error.message
      );
 ...
 */
function buildPromotedTextEvent(
  event: SSEEvent,
  timestamp: number
): TextTimelineEvent {
  return {
...
export function fromSupabaseRow(row: SupabaseRow): UnifiedTimelineEvent[] {
  ...
  if (typeof row.content === "string") {
    ...
    if (row.role === "tool") {
      ...
      (toolResultEvent as EnhancedToolResultEvent).messageId =
      ...
      if (contentPart.type === "text") {
        ...
        const toolCallEvent: ToolCallTimelineEvent = {
          ...
          toolCall: {
            ...
            args:
              contentPart.arguments ||
              contentPart.function?.arguments ||
              contentPart.input ||
              contentPart.args ||
              {},
          },
        ...
        const toolCallId =
          contentPart.tool_use_id || // <-- This is the main one for user messages!
        ...
        if (
          (contentPart as any).content &&
          Array.isArray((contentPart as any).content)
        ) {
          // For user role messages, content is an array with text objects
          const textContent = (contentPart as any).content.find(
            (c: any) => c.type === "text"
          );
          if (textContent) {
...
export function fromSSEEvent(event: SSEEvent): UnifiedTimelineEvent | null {
  ...
  const isCompleteChunk = (ev: SSEEvent): boolean => {
    return (
      (ev.type === "chunk" || ev.type === "message_chunk") && !!ev.data?.content
    );
  };
  ...
  switch (event.type) {
    ...
    case "message_chunk": {
      ...
      if (!event.delta) {
        logDebug("skip_shape_mismatch_chunk", {
          event,
    ...
    case "tool_call":
      ...
      if (!toolCallData) {
        logDebug("skip_shape_mismatch_tool_call", {
          event,
          reason: "missing data.tool_call",
        });
        return null;
      }
    ...
    case "tool_result":
      ...
      if (!toolResultData) {
        logDebug("skip_shape_mismatch_tool_result", {
          event,
          reason: "missing data",
        });
        return null;
    ...
    default:
      ...
      logDebug("skip_unknown_event_type", {
        event,
        reason: `unknown type: ${event.type}`,
      });
      return null;
  }
}
 ...
 */
export const toUnifiedEvents = (
  src: SupabaseRow | ChatMessage | SSEEvent | SupabaseRow[] | ChatMessage[]
): UnifiedTimelineEvent[] => {
...
export const mergeTimelineEvents = (
  ...
  for (const event of fixedEvents) {
    ...
    let key =
      eventId ||
      event.id ||
      (messageId ? `${messageId}:${event.type}` : `${event.id}:${event.type}`);
    ...
    if (eventId && seen.has(eventId)) {
      ...
      const existingEvent = merged.find(
        (e) => (e as TimelineEventWithMetadata).event_id === eventId
      );
