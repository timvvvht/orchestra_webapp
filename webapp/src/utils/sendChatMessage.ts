...
import { createSessionFast, CreateSessionOptions } from '@/hooks/useCreateSessionFast';
...
export async function sendChatMessage(params: SendChatMessageParams): Promise<SendChatMessageResult> {
    ...
    console.log(`📤 [sendChatMessage] [${messageId}] Images count: ${images.length}`);
    // amazonq-ignore-next-line
    ...
    console.log(`🆕 [sendChatMessage] [${messageId}] Ensuring session exists in database...`);
    ...
    try {
        ...
        if (params.acsOverrides?.agent_cwd_override) {
            sessionCreationOptions.workspacePath = params.acsOverrides.agent_cwd_override;
        }
        ...
        console.log(`🆕 [sendChatMessage] [${messageId}] Creating/ensuring session with options:`, sessionCreationOptions);

        const sessionResult = await createSessionFast(sessionCreationOptions);

        if (!sessionResult.success || !sessionResult.sessionId) {
            console.error(`❌ [sendChatMessage] [${messageId}] Failed to create/ensure session:`, sessionResult.error);
        ...
        console.log(`✅ [sendChatMessage] [${messageId}] Session ensured in database: ${effectiveSessionId}`);
    } catch (error) {
        console.error(`❌ [sendChatMessage] [${messageId}] Exception during session creation:`, error);
        toast.error('Failed to create chat session', {
    ...
    console.log(`✅ [sendChatMessage] [${messageId}] Validation passed, using session ID: ${effectiveSessionId}`);

    // Register tools (defaults to core tools if not specified)
    const toolsToRegister = params.tools || ['search_files', 'cat', 'str_replace_editor'];
    ...
    try {
        ...
        console.log(`📝 [sendChatMessage] [${messageId}] User message created:`, {
            ...
            contentLength: userMessage.content[0]?.type === 'text' ? userMessage.content[0].text?.length || 0 : 0,
            imagesCount: userMessage.images?.length || 0
        });
        console.log(`🖼️ [sendChatMessage] [${messageId}] User message images:`, userMessage.images);
        ...
        const resolvedUseStoredKeys = params.useStoredKeys ?? useBYOKStore.getState().useStoredKeysPreference;
        ...
        if (isWebOrigin) {
            ...
            try {
                ...
                console.error(`❌ [sendChatMessage] [${messageId}] Failed to build payload:`, error);
                throw error;
            }
        }
        ...
        console.log(`🖼️ [sendChatMessage] [${messageId}] Images in payload:`, body.images);

        // 5) POST to ACS Converse using httpApi
        const ACS_BASE = import.meta.env.VITE_ACS_BASE_URL || 'http://localhost:8001';
        ...
        // Determine auth for web-origin (hard requirement: header OR cookie)
        const { headers, credentials: credentialsOpt } = await determineWebAuth(isWebOrigin, ACS_BASE, messageId);
        ...
        console.log(`🖼️ [sendChatMessage] [${messageId}] IMAGES IN REQUEST BODY:`, body.images);
        ...
        const res = await httpApi.POST<ConverseResponse>(url, {
            headers,
            body,
            ...(credentialsOpt ? { credentials: credentialsOpt } : {})
        });
        ...
        if (!res || !res.ok) {
            ...
            console.error(`❌ [sendChatMessage] [${messageId}] ACS request failed:`, errorDetails);
            throw new Error(`ACS converse error ${res?.status ?? 'unknown'}: ${errText}`);
        }
        ...
        console.error(`💥 [sendChatMessage] [${messageId}] Failed to send message:`, errorDetails);
        const sanitizedErrorMessage = error instanceof Error ? error.message.replace(/[\r\n\t]/g, ' ').substring(0, 200) : 'Unknown error';
...
// Decide auth mode for web-origin: Authorization header or cookie (credentials: 'include')
async function determineWebAuth(
    isWebOrigin: boolean,
    acsBase: string,
    messageId: string
    ...
    if (isWebOrigin) {
        headers['X-Orchestra-Web-Origin'] = 'true';
        // 1) Try Supabase JWT for Authorization header
        try {
            const {
                data: { session }
            ...
            if (session?.access_token) {
                ...
                console.log(`🔐 [sendChatMessage] [${messageId}] Using Authorization header (Supabase JWT)`);
                return { headers };
            }
        } catch (e) {
            console.warn(`⚠️ [sendChatMessage] [${messageId}] Unable to read Supabase session for Authorization`, e);
        }
        // 2) Fallback to cookie-based session: validate via /api/v1/auth/me
        try {
            const me = await httpApi.GET(`${acsBase}/api/v1/auth/me`, { credentials: 'include' });
            if (me?.ok) {
                console.log(`🔐 [sendChatMessage] [${messageId}] Using cookie auth (credentials: 'include')`);
                return { headers, credentials: 'include' };
            }
        } catch (e) {
            console.warn(`⚠️ [sendChatMessage] [${messageId}] Cookie auth validation failed`, e);
        }
        // 3) No auth detected → hard fail
        const err = 'Authentication required: supply Authorization (Supabase) or call exchange to set ACS cookies.';
        console.error(`❌ [sendChatMessage] [${messageId}] ${err}`);
        throw new Error(err);
    }
    // Non-web: no special requirement
    return { headers };
}
...
function buildConversePayload(params: SendChatMessageParams, effectiveAgentCwd?: string): ConverseRequest {
    const {
        sessionId,
        message,
        userId,
...
type WebConverseRequest = {
    // ACSWebConverseRequest superset (user_id omitted for web)
    agent_config_name: string;
    prompt: string;
    session_id?: string;
    repo_id: number;
