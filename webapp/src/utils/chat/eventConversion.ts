...
import { isCheckpointEvent } from '@/types/events';
      ...
      if (Array.isArray(event.content)) {
        ...
        content = event.content.map((part: any) => {
          ...
          if (part.type === 'text' && part.text === '[object Object]') {
            ...
            if (part.content) {
              const recoveredText = typeof part.content === 'string' ? part.content : JSON.stringify(part.content);
              return { ...part, text: recoveredText };
            }
            
            // Check if there are other fields that might contain the text
            const possibleFields = ['message', 'value', 'data', 'body', 'original'];
            for (const field of possibleFields) {
              if (part[field]) {
                const recoveredText = typeof part[field] === 'string' ? part[field] : JSON.stringify(part[field]);
                return { ...part, text: recoveredText };
              }
            }
            ...
            return { ...part, text: '[Content corrupted in database - the original text was incorrectly stored as "[object Object]"]' };
          }
          return part;
        });
      } else if (event.content && typeof event.content === 'object' && event.content.content) {
        ...
        if (Array.isArray(event.content.content)) {
          content = event.content.content.map((part: any) => {
            // Fix existing "[object Object]" issues
            if (part.type === 'text' && part.text === '[object Object]') {
              return { ...part, text: '[Content parsing error - click "🔄 Refresh Data" to fix]' };
            }
            return part;
          });
        } else {
 ...
 */
export async function convertEventsToMessagesAsync(events: any[], batchSize: number = 50): Promise<ChatMessage[]> {
  // PERFORMANCE: Early exit for empty events
  if (!events || events.length === 0) {
    return [];
  }
