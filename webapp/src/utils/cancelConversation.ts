...
export async function cancelConversation(sessionId: string): Promise<void> {
    console.log('🚫 [CANCEL-UTIL] Starting conversation cancellation for session:', sessionId);
    ...
    // Mark session as cancelling immediately
    useSessionStatusStore.getState().markCancelling(sessionId, 'user_cancel');
    ...
    try {
        ...
        console.log('🚫 [CANCEL-UTIL] 📡 Making POST request to /acs/converse/cancel');
        console.log('🚫 [CANCEL-UTIL] 📤 Request payload:', { session_id: sessionId });
        ...
        setTimeout(() => {
            const currentStatus = useSessionStatusStore.getState().getStatus(sessionId);
            if (currentStatus.state === 'cancelling') {
                console.log('🚫 [CANCEL-UTIL] Fallback: No server update after cancel, marking idle');
        ...
        throw new Error(`Failed to cancel conversation: ${error instanceof Error ? error.message : String(error)}`);
    }
}