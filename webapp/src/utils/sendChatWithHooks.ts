...
} from "@/hooks/chatLifecycleHooks";

export interface SendChatWithHooksOptions extends SendChatMessageParams {
  cwd?: string;
}

export async function sendChatWithHooks(
  options: SendChatWithHooksOptions
  ...
  try {
    // Run pre-chat hooks
    console.log(
      `[sendChatWithHooks] Running pre-chat hooks for session ${sessionId}, with ctx: ${JSON.stringify(ctx, null, 2)}`
    );
