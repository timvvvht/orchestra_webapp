...
import { ChevronRight, Archive, ArchiveRestore, Loader2, FileText, Lock, Clock, GitBranch } from 'lucide-react';
import { MissionControlAgent, useMissionControlStore } from '@/stores/missionControlStore';
...
function getAssistantPreview(raw: string): string {
  ...
  const firstSentence = firstLine.split(/[.!?]/).find(s => s.trim())?.trim() || firstLine;
...
const getHumanReadableActivity = (
  ...
  text: string;
  tone: 'exploring' | 'creating' | 'thinking' | 'testing' | 'reviewing' | 'complete';
} => {
  ...
  if (role === 'tool_call') {
    const toolName = (typeof content === 'string'
      ? content
    ...
    const editingTools = ['apply_patch', 'str_replace_editor', 'write_file', 'create_file', 'delete_file'];
    const runningTools = ['execute_in_runner_session', 'run_command', 'initiate_runner_session'];
    ...
    if (thinkingTools.some(t => toolName.includes(t))) {
      return { text: toolName.includes('plan') ? 'Planning approach…' : 'Thinking…', tone: 'thinking' };
    }
...
const AgentCard: React.FC<AgentCardProps> = ({
  ...
  const { selectedSession, setSelectedSession, markSessionRead, isSessionUnread } = useMissionControlStore();
  ...
  const messagePreview = useMemo(() => getLatestMessagePreview(agent, isInSplitScreen), [agent, isInSplitScreen]);
  const timeAgo = useMemo(() => formatTimeAgo(getSessionTimestamp(agent)), [agent]);
  const codePath = useMemo(() => formatCodePath(agent.base_dir ?? agent.agent_cwd), [agent.base_dir, agent.agent_cwd]);
  
  // Worktree detection
  const isWorktree = useMemo(() => isWorktreePath(agent.agent_cwd || agent.base_dir), [agent.agent_cwd, agent.base_dir]);
  const worktreeName = useMemo(() => getWorktreeName(agent.agent_cwd || agent.base_dir), [agent.agent_cwd, agent.base_dir]);
  ...
  const glowStyles = {
    'needs-review': 'bg-white/[0.02] border-amber-400/[0.08] hover:border-amber-400/[0.16] hover:bg-white/[0.05]',
    'processing': 'bg-white/[0.02] border-blue-400/[0.08] hover:border-blue-400/[0.18] hover:bg-white/[0.05] hover:shadow-[0_0_32px_rgba(0,119,237,0.06)]',
    'idle': isUnread ? 'bg-white/[0.02] border-emerald-400/[0.12] hover:border-emerald-400/[0.18] hover:bg-white/[0.04]' : 'bg-white/[0.01] border-white/[0.08] hover:border-white/[0.16] hover:bg-white/[0.03]'
  };

  // Special styling for creating status, pending sessions, or background processing
  const getCardStyles = () => {
    if (agent.status === 'creating' || agent.isPending || agent.backgroundProcessing) {
    ...
    >
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          "bg-gradient-to-br",
          agent.status === 'processing' ? "from-blue-500/10 to-transparent" :
          agent.status === 'creating' ? "from-emerald-500/10 to-transparent" :
          "from-white/5 to-transparent"
        )} />

        {/* Activity-based glow for active states */}
        {(agent.status === 'creating' || agent.isPending || agent.backgroundProcessing) && (
          <div className="absolute inset-0 rounded-2xl bg-emerald-500/[0.05] animate-pulse" />
        )}
        {group === 'processing' && agent.status !== 'creating' && !agent.isPending && !agent.backgroundProcessing && (
        ...
        <div className={`relative px-6 py-5`}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {/* Unread indicator */}
              {isUnread && (
                <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" title="Unread" />
              )}
              {/* {plan && (
                <button
                  onClick={(e) => {
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-white/60 font-mono truncate" title={agent.agent_cwd || undefined}>
          ...
          >
            ...
            {!agent.isPending && !agent.backgroundProcessing && (
              showArchived ? (
                <button
