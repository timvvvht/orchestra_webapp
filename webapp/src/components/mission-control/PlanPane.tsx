...
const PlanPane: React.FC<PlanPaneProps> = ({ sessionId }) => {
  ...
  const markdownComponents = useMemo(
    () => ({
      ...
      p: ({ children }: any) => (
        <p className="my-4 leading-[1.75] tracking-[-0.011em]">{children}</p>
      ),
            ...
            <div className="relative rounded-lg overflow-hidden border border-white/[0.06] bg-zinc-900/50 backdrop-blur-sm w-full max-w-full min-w-0 contain-layout contain-style">
              ...
              <button
                ...
                className={cn(
                  ...
                  copiedId === codeId &&
                    "opacity-100 bg-green-500/20 border-green-500/30"
                )}
      ...
      ol: ({ children }: any) => (
        <ol className="my-4 ml-6 space-y-2">{children}</ol>
      ),
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        ...
        >
          ...
          <p className="text-lg font-light text-white/40 mb-2">
            No Plan Available
          </p>
          <p className="text-sm text-white/30">
            This session awaits its blueprint.
          </p>
        </motion.div>
      </div>
    );
