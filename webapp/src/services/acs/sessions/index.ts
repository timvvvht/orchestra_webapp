...
export class ACSSessionService {
  ...
  async createDefaultSession(
    ...
    options?: { agentCwd?: string } & RequestOptions
  ): Promise<APIResponse<SessionResponse>> {
  ...
  private detectProjectPath(): string | null {
    try {
      ...
      if (
        ...
        (window as any).__ORCHESTRA_PROJECT_PATH__
      ) {
  ...
  private getUserHomeDirectory(): string | null {
    try {
      ...
      console.warn(
        "[ACSSessionService] Failed to detect home directory:",
        error
      );
      return null;
    }
  ...
  async getSession(
    ...
    console.log(
      `🔧 [ACSSessionService] getSession endpoint: ${endpoint} - sessionId: ${sessionId}`
    );
    ...
    const response = await this.client.get<SessionDetails>(
      `${endpoint}${query}`,
      requestOptions
    );
    console.log(
      `🔧 [ACSSessionService] getSession response:`,
  ...
  async updateSession(
    ...
    try {
      const result = await this.client.put<SessionResponse>(
        endpoint,
        updates,
        options
      );
  ...
  async cleanupOldSessions(
    ...
    return this.client.post<SessionResponse>(
      `${ACS_ENDPOINTS.SESSIONS_CLEANUP}${query}`,
      undefined,
      options
    );
  }
  ...
  async searchSessions(
    ...
    return this.client.get<SessionListResponse>(
      `${ACS_ENDPOINTS.SESSIONS}${searchQuery}`,
      options
    );
  }
}
...
export class SessionUtils {
   ...
   */
  static filterByAgent(
    sessions: SessionSummary[],
    agentConfigId: string
  ): SessionSummary[] {
    return sessions.filter(
      (session) => session.agent_config_id === agentConfigId
    );
  }
