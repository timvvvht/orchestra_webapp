...
export class ACSCoreService {
     ...
     */
    async converse(request: ACSConverseRequest, options?: RequestOptions): Promise<APIResponse<ACSConverseResponse>> {
        ...
        return this.client.post<ACSConverseResponse>(ACS_ENDPOINTS.CONVERSE, request, converseOptions);
    }
    ...
    async sendMessage(
        ...
        const { modelApiKeys, useStoredKeys, overrides, templateVariables, agentCwd, autoMode, modelAutoMode, ...requestOptions } = options || {};
        ...
        console.log('🎯 [ACSCoreService.sendMessage] MISSION_CONTROL_AUTO_MODE_DEBUG - Full payload:', JSON.stringify(request, null, 2));
    ...
    async startConversation(
        ...
        const { agentCwd, modelApiKeys, useStoredKeys, sessionId, overrides, templateVariables, autoMode, modelAutoMode, ...requestOptions } = options || {};

        // 🚨 CRITICAL: Validate sessionId to prevent string "undefined" reaching backend
        let validSessionId = sessionId;
        if (sessionId === 'undefined' || sessionId === 'null') {
            console.warn('⚠️ [ACSCoreService] Detected string "undefined" or "null" as sessionId, converting to null');
            validSessionId = null;
        }
        ...
        console.log('🚀 [ACSCoreService.startConversation] Final request payload:', {
    ...
    async converseWithKeys(
        ...
        const { useStoredKeys, templateVariables, ...requestOptions } = options || {};
     ...
     */
    async pingTES(userId?: string, options?: RequestOptions): Promise<APIResponse<any>> {
     ...
     */
    async testToolRouting(toolName: string, options?: RequestOptions): Promise<APIResponse<any>> {
        return this.client.get(`/acs/routing/test/${encodeURIComponent(toolName)}`, options);
    }

    /**
     * Reload the YAML routing configuration
     */
    async reloadRoutingConfig(options?: RequestOptions): Promise<APIResponse<any>> {
     ...
     */
    async cancelConversation(sessionId: string, options?: RequestOptions): Promise<APIResponse<void>> {
        return this.client.post<void>('/acs/converse/cancel', { session_id: sessionId }, options);
    }
}
...
export class ConverseResponseUtils {
    ...
    static hasToolCalls(response: ACSConverseResponse): boolean {
        const messages = response.response_messages || [];
        return messages.some((msg: any) => msg.content && Array.isArray(msg.content) && msg.content.some((part: any) => part.type === 'tool_use'));
    }
    ...
    static isComplete(response: ACSConverseResponse): boolean {
        ...
        if (Array.isArray(lastMessage.content)) {
            const hasPendingToolCall = lastMessage.content.some((part: any) => part.type === 'tool_use' && !part.result);
