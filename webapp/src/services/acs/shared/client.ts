import type { ACSClientConfig, RequestOptions, APIResponse, HTTPMethod } from './types';
import { JWTTokenManager } from '@/lib/jwtTokenManager';
import { ACSTokenInterceptor, TokenRefreshStrategies } from './tokenInterceptor';
...
export class ACSClient {
    ...
    setAuthToken(token: string): void {
        ...
        if (token && token.length > 600) {
            console.warn('⚠️ [ACSClient] Token unexpectedly long after helper filter (length: ' + token.length + ') - this should not happen');
            console.warn('💡 [ACSClient] The getSupabaseAccessToken helper should have filtered this out');
        }
     ...
     */
    async request<T = any>(method: HTTPMethod, endpoint: string, data?: any, options: RequestOptions = {}): Promise<APIResponse<T>> {
     ...
     */
    private buildRequestOptions(method: HTTPMethod, data?: any, options: RequestOptions = {}): RequestInit {
        ...
        if (this.authToken) {
            ...
            console.log('⚠️ [ACSClient] No auth token available, Authorization header NOT added');
        }
        ...
        // also enabled.
        // ---------------------------------------------------------------------------
        if (typeof process !== 'undefined' && process.env?.DISABLE_RATE_LIMIT === '1') {
    ...
    private async handleResponse<T>(response: Response): Promise<APIResponse<T>> {
        ...
        if (!response || !response.headers) {
            console.log(`❌ ACS Invalid response object received`, JSON.stringify(response, null, 2));
            throw new ACSError('Invalid response object', 0, 'NETWORK_ERROR');
        }

        console.log(`📋 ACS Response status: ${response.status} ${response.statusText}`);
        ...
        try {
            ...
            console.log(`❌ ACS Failed to parse response: ${(error as Error).message}`);
            throw new ACSError(`Failed to parse response: ${(error as Error).message}`, 0, 'PARSE_ERROR');
        }
        ...
        if (!response.ok) {
            ...
            if (typeof data === 'object' && data !== null) {
                ...
                if (errorObj.detail) {
                    errorDetail = typeof errorObj.detail === 'string' ? errorObj.detail : JSON.stringify(errorObj.detail);
            ...
            const statusCode = response.status ?? (typeof data?.status === 'number' ? data.status : 500);
