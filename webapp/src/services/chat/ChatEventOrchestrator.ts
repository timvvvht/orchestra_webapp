...
export class ChatEventOrchestrator {
  ...
  constructor(firehose: FirehoseMux, config: ChatEventOrchestratorConfig = {}) {
    ...
    console.log(
      "💬 [ChatEventOrchestrator] Initialized with config:",
      this.config
    );
  ...
  start(): void {
    ...
    console.log(
      "💬 [ChatEventOrchestrator] 🚀 Starting chat event orchestrator"
    );
    ...
    this.isStarted = true;
    console.log(
      "💬 [ChatEventOrchestrator] ✅ Now listening for chat events from user-specific SSE stream"
    );
  }
  ...
  stop(): void {
    ...
    console.log(
      "💬 [ChatEventOrchestrator] 🛑 Stopping chat event orchestrator"
    );
  ...
  private handleRawFirehoseEvent(rawEvent: ACSRawEvent): void {
    ...
    if (this.dedupe.seen(rawEvent.event_id)) {
      if (this.config.debugEnabled) {
        console.log(
          "💬 [ChatEventOrchestrator] ⏭️ Skipping duplicate event:",
          rawEvent.event_id
        );
      }
      return;
  ...
  private processBatch(): void {
    ...
    const eventsByType = eventsToProcess.reduce(
      (acc, event) => {
  ...
  private processSSEEvent(event: SSEEvent): void {
    ...
    if (!event.sessionId) {
      ...
      if (this.config.debugEnabled) {
        console.log(
          "💬 [ChatEventOrchestrator] ⚠️ Skipping event without sessionId:",
          event
        );
      }
      return;
    ...
    try {
      ...
      unifiedEvents.forEach((unifiedEvent) => {
        ...
        if (unifiedEvent.type === "completion_signal") {
          ...
          sessionEvents.forEach((eventId) => {
            ...
            if (event && event.kind === "message" && event.partial) {
              ...
              console.log(
                "[ChatEventOrchestrator] ADDING event (completion_cleanup)",
                {
                  id: updatedEvent.id,
                  sessionId,
        ...
        if (unifiedEvent.type === "chunk") {
          ...
          if (existingEvent && existingEvent.kind === "message") {
            ...
            const existingText =
              existingEvent.content?.[0]?.type === "text"
                ? existingEvent.content[0].text
                : "";
          ...
          if (unifiedEvent.type === "text") {
            ...
            createdAt = new Date(
              textEvent.createdAt || unifiedEvent.timestamp || Date.now()
            ).toISOString();
          } else {
            ...
            content = messageData?.content || [
              { type: "text", text: messageData?.text || "" },
            ];
            partial = messageData?.isStreaming || false;
            createdAt = new Date(
              unifiedEvent.timestamp || Date.now()
            ).toISOString();
          }
      ...
      console.error(
        "💬 [ChatEventOrchestrator] ❌ Failed to process SSE event:",
        error
      );
  ...
  private flushBatch(): void {
    if (this.eventBatch.length > 0) {
      console.log(
        "💬 [ChatEventOrchestrator] 🚿 Flushing remaining batch events:",
