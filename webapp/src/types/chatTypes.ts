...
export interface ChatMessage {
    ...
    delivered?: boolean;
    read?: boolean;
    status?: 'optimistic' | 'sending' | 'sent' | 'error' | 'delivered_to_rust' | 'persisted_db';
    error?: string;
...
export type DbNewChatSession = Partial<Pick<SupabaseDbChatSession, 'name' | 'avatar_url' | 'agent_config_id' | 'metadata' | 'user_id' | 'parent_session_id' | 'fork_message_id' | 'display_title' | 'agent_cwd'> & { id?: string; initial_agent_snapshot?: Record<string, any> | null }>;
export type DbUpdateChatSession = Partial<Pick<SupabaseDbChatSession, 'name' | 'avatar_url' | 'metadata' | 'last_message_at'> & { initial_agent_snapshot?: Record<string, any> | null }>;
