...
async function hydrateCheckpoints(sessionId: string): Promise<void> {
    try {
        ...
        const sortedCheckpoints = [...checkpointsWithHash].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
        ...
        for (const checkpoint of sortedCheckpoints) {
            ...
            if(mostRecentCommitHash) console.log(`[HistoryBridge] 📝 Updated mostRecentCommitHash to ${mostRecentCommitHash.slice(0, 8)}`);
        }
