...
export const useMissionControlFirehose = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = firehose.subscribe((rawEvent: any) => {
            ...
            console.log('[plan] SSE event received - type:', eventType, 'session:', sessionId);
            console.log('[plan] SSE event received - type:', eventType, 'session:', sessionId);
            ...
            switch (eventType) {
                ...
                case 'tool_result': {
                    ...
                    if (toolName.startsWith('plan_') || toolId.startsWith('plan_')) {
                        ...
                        console.log('[plan] SSE event calling refetchSinglePlan for session:', sessionId);
                ...
                case 'done':
                    ...
                    console.log('[plan] SSE event calling refetchPlans for session end:', sessionId);
                    refetchPlans();
                    break;
    ...
    const scheduleDebouncedRefetchSingle = (sid: string) => {
        ...
        const h = window.setTimeout(() => {
            ...
            console.log('[plan] Debounced SSE event calling refetchSinglePlan for session:', sid);
    ...
    const scheduleDebouncedRefetch = (sessionId: string) => {
        ...
        const handle = setTimeout(() => {
            ...
            console.log('[plan] Debounced SSE event calling refetchPlans for session:', sessionId);
            refetchPlans();
        }, 3000);
