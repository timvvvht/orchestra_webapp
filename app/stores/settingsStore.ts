...
interface SettingsState {
  ...
  setThemeSetting: <K extends keyof ThemeSettings>(
    key: K,
  ...
  setVaultSetting: <K extends keyof VaultSettings>(
    key: K,
  ...
  setKeyboardSetting: <K extends keyof KeyboardSettings>(
    key: K,
  ...
  setNotificationSetting: <K extends keyof NotificationSettings | string[]>(
    key: K,
    value: any
  ) => Promise<void>;
  setNotificationSettings: (
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      // Theme settings actions
      setThemeSetting: async <K extends keyof ThemeSettings>(
        key: K,
        value: ThemeSettings[K]
      ) => {
        ...
        try {
          ...
          // Revert on error
          console.error("[settingsStore] setVaultSetting failed", {
            key,
            value,
            error,
          });
      ...
      debouncedSetThemeSettings: debounce(
      ...
      // Vault settings actions
      setVaultSetting: async <K extends keyof VaultSettings>(
        key: K,
        value: VaultSettings[K]
      ) => {
        console.log("[settingsStore] setVaultSetting called", {
          key,
          value,
          type: typeof value,
        });
        ...
        if (key === "path" && typeof value !== "string") {
          console.warn(
            "[settingsStore] Vault path is not a string, attempting to extract:",
          ...
          if (typeof value === "object" && value !== null) {
            ...
            if ("path" in value && typeof (value as any).path === "string") {
              ...
              console.log(
                "[settingsStore] Extracted path from object:",
                processedValue
              );
            } else if (
              "data" in value &&
              ...
              console.log(
                "[settingsStore] Extracted data from object:",
                processedValue
              );
            } else {
              console.error(
                "[settingsStore] Could not extract string path from object:",
                value
              );
            ...
            console.log(
              "[settingsStore] Converted value to string:",
              processedValue
            );
          }
        }
        ...
        try {
          // Save to backend
          console.log("[settingsStore] Calling setPreference for vault:", {
            key,
            value: processedValue,
          });
          await setPreference(`vault.${key}`, processedValue);
          console.log("[settingsStore] setPreference completed:", {
            key,
            value: processedValue,
          });
          set({ isLoading: false });
      ...
      debouncedSetVaultSettings: debounce(
        async (vaultSettings: Partial<VaultSettings>) => {
      ...
      // Keyboard settings actions
      setKeyboardSetting: async <K extends keyof KeyboardSettings>(
        key: K,
        value: KeyboardSettings[K]
      ) => {
        // Update local state optimistically
      ...
      debouncedSetKeyboardSettings: debounce(
        async (keyboardSettings: Partial<KeyboardSettings>) => {
      ...
      setNotificationSetting: async <
        K extends keyof NotificationSettings | string[],
      >(
        key: K,
        value: any
      ) => {
      ...
      debouncedSetNotificationSettings: debounce(
      ...
      // UI settings actions
      setUiSetting: async <K extends keyof UiSettings>(
        key: K,
        value: UiSettings[K]
      ) => {
        // Update local state optimistically
      ...
      debouncedSetUiSettings: debounce(
        async (uiSettings: Partial<UiSettings>) => {
      ...
      initSettings: async () => {
        ...
        try {
          ...
          if (directVaultPath && !loadedSettings.vault.path) {
            console.log(
              "[settingsStore] Found vault.path as direct key:",
              directVaultPath
            );
          ...
          loadedSettings.keyboard = await getKeyboardSettings(
            defaultSettings.keyboard
          );

          // Load notification settings
          loadedSettings.notifications = await getNotificationSettings(
          ...
          // Load version
          const version = await getPreference(
            "version",
          ...
          const migratedSettings =
            await migrationRunner.migrateSettings(mergedSettings);
