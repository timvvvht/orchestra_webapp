...
export const useDraftStore = create<DraftStoreState>((set, get) => ({
  ...
  addDraft: (draftPartial) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(
        "orchestra_drafts",
        JSON.stringify({ ...get().drafts, [id]: newDraft })
      );
    }
    return id;
  },
  ...
  getDraftsArray: () =>
    Object.values(get().drafts).sort((a, b) => b.createdAt - a.createdAt),
}));
    ...
    try {
      ...
      Object.entries(parsed).forEach(([id, draft]) => {
        ...
        if (
          ...
          typeof draft.roleModelOverrides === "object"
        ) {
          // Filter out any invalid role keys (for backward compatibility)
          const validRoles: (keyof RoleModelOverrides)[] = [
            "explore",
            "plan",
            "execute",
            "debug",
          ...
          validRoles.forEach((role) => {
            if (
              draft.roleModelOverrides &&
              role in draft.roleModelOverrides &&
              typeof draft.roleModelOverrides[role] === "string"
            ) {
          ...
          const draftWithOverrides: DraftIssue = {
            ...
            ...(Object.keys(cleanedOverrides).length > 0
              ? { roleModelOverrides: cleanedOverrides }
              : {}),
          };
