 ...
 */
export async function startBackgroundSessionOps(sessionId: string, options: SessionBackgroundWorkerOptions): Promise<void> {
    console.log(`[sessionBackgroundWorker] options.auto_mode: ${options.auto_mode} - options.model_auto_mode: ${options.model_auto_mode}`);
    const {
        agentConfigId,
        userId,
        projectRoot,
    ...
    console.log(`[sessionBackgroundWorker] auto_mode: ${autoMode} - model_auto_mode: ${modelAutoMode}`);
    ...
    const skipPrep =
        typeof skipWorkspacePreparation === 'boolean'
            ? skipWorkspacePreparation
            : (typeof enableWorktrees === 'boolean' ? !enableWorktrees : false);

    console.log(`[sessionBackgroundWorker] skipPrep: ${skipPrep} (skipWorkspacePreparation: ${skipWorkspacePreparation}, enableWorktrees: ${enableWorktrees})`);

    // Log role model overrides for debugging
    if (Object.keys(roleModelOverrides).length > 0) {
        console.log('🔧 [sessionBackgroundWorker] Using role model overrides:', roleModelOverrides);
    }

    let currentProgress = 0;
    const totalWeight = BACKGROUND_STEPS.reduce((sum, step) => sum + step.weight, 0);
    ...
    const updateProgress = (stepName: string, stepProgress: number) => {
        ...
        if (step && onProgress) {
            ...
            const totalProgress = Math.min(100, ((currentProgress + progressIncrement) / totalWeight) * 100);
    ...
    try {
        ...
        async function resolveWorkspace(sessionId: string, originalPath: string): Promise<string> {
            let resolved = originalPath;
            const isGit = await isGitRepo(originalPath);

            if (isGit) {
                try {
                    resolved = await taskOrchestration.prepareTaskWorkspace(sessionId, originalPath);
                    console.log(`✅ [SessionBackgroundWorker] Worktree created: ${resolved}`);
                } catch (error) {
                    ...
                    if (enableWorktrees) {
                        console.error('[SessionBackgroundWorker] Worktree creation failed in isolation mode, aborting', error);
                    ...
                    console.warn('[SessionBackgroundWorker] Worktree creation failed, fallback to original path', error);
                    resolved = originalPath;
                }
            } else {
                console.log(`✅ [SessionBackgroundWorker] Non-git path, using original: ${originalPath}`);
            }
            return resolved;
        }
        ...
        try {
            ...
            console.log(`✅ [SessionBackgroundWorker] agent_cwd updated to: ${resolvedCwd}`);

            // Set base_dir to the original project path if provided
            if (originalProjectPath) {
                await taskOrchestration.setSessionBaseDir(sessionId, originalProjectPath);
                console.log(`✅ [SessionBackgroundWorker] base_dir set to original project path: ${originalProjectPath}`);
            }
        ...
        console.log('[SessionBackgroundWorker] Sending first message:', JSON.stringify(firstMessage));
        if (firstMessage) {
            ...
            const messageParams: Record<string, unknown> = {
                ...
                userId,
                agentConfigName: agentConfigId,
                acsClient: getDefaultACSClient(),
                cwd: resolvedCwd, // Pass cwd to hook context
                acsOverrides: {
                    ...(acsOverrides || {}),
                    // Only set agent_cwd_override if not already provided in acsOverrides
                    ...(acsOverrides?.agent_cwd_override ? {} : { agent_cwd_override: resolvedCwd })
                },
            ...
            messageParams.cwd = resolvedCwd;
            console.log(`[SessionBackgroundWorker][sendChatMessage] Sending auto_mode: ${autoMode}, model_auto_mode: ${modelAutoMode}`);

            const result = await taskOrchestration.sendInitialTaskMessage(messageParams);
