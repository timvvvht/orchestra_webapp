 ...
 */
export const isTextEvent = (
  event: UnifiedTimelineEvent
): event is TextTimelineEvent => event.type === "text";

export const isToolCallEvent = (
  event: UnifiedTimelineEvent
): event is ToolCallTimelineEvent => event.type === "tool_call";

export const isToolResultEvent = (
  event: UnifiedTimelineEvent
): event is ToolResultTimelineEvent => event.type === "tool_result";

export const isChunkEvent = (
  event: UnifiedTimelineEvent
): event is ChunkTimelineEvent => event.type === "chunk";

export const isThinkGroupEvent = (
  event: UnifiedTimelineEvent
): event is ThinkGroupTimelineEvent => event.type === "think_group";

export const isToolInteractionEvent = (
  event: UnifiedTimelineEvent
): event is ToolInteractionTimelineEvent => event.type === "tool_interaction";
...
export const hasStreamingEvents = (events: UnifiedTimelineEvent[]): boolean =>
  events.some(
    (event) =>
      (isChunkEvent(event) || isTextEvent(event)) && event.isStreaming === true
  );
...
export const getStreamingMessageId = (
  ...
  const streamingChunk = events
    .filter(isChunkEvent)
    .find((event) => event.isStreaming);
 ...
 */
export const consolidateChunks = (
  events: UnifiedTimelineEvent[]
