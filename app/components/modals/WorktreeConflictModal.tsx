...
import type { DiffFile } from "@/components/monaco/AdvancedMonacoDiffViewer";
...
export const WorktreeConflictModal: React.FC<WorktreeConflictModalProps> = ({
  ...
  useEffect(() => {
    if (!isOpen) return;
    (async () => {
      const files = await buildDiffFilesFromConflicts(
        worktreePath,
        conflictedFiles
      );
      setDiffFiles(files);
  ...
  const isResolved = (file: DiffFile) =>
    !stripGitConflictMarkers(file.currentContent).hadMarkers;
  ...
  const handleCompleteMerge = useCallback(async () => {
    try {
      ...
      console.log(
        `[WorktreeConflictModal] Completing merge for session: ${sessionId}`
      );
      console.log(
        `[WorktreeConflictModal] All files resolved, calling onResolvedAll callback`
      );

      await onResolvedAll();

      console.log(
        `[WorktreeConflictModal] Merge completed successfully for session: ${sessionId}`
      );
      toast.success("Merge completed successfully");
    } catch (error) {
      console.error(
        `[WorktreeConflictModal] Failed to complete merge for session ${sessionId}:`,
        error
      );
