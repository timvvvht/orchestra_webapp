...
const CheckpointsPane: React.FC<CheckpointsPaneProps> = ({
  sessionId,
  agent,
}) => {
  ...
  const [checkpointFileChanges, setCheckpointFileChanges] = useState<
    FileChange[]
  >([]);
  ...
  const [firstCheckpointHash, setFirstCheckpointHash] = useState<string | null>(
    null
  );
    ...
    try {
      ...
      if (data && agent.agent_cwd) {
        ...
        if (
          data.length > 0 &&
          data[0].commit_hash &&
          data[data.length - 1].commit_hash
        ) {
          loadOverallSessionStats(
            data[0].commit_hash,
            data[data.length - 1].commit_hash
          );
        }
  ...
  const loadOverallSessionStats = async (
    latestHash: string,
    earliestHash: string
  ) => {
    ...
    try {
      ...
      const fromHash =
        latestHash === earliestHash ? `${earliestHash}~1` : earliestHash;
  ...
  const handleMergeCheckpoint = async (
    commitHash: string,
    e: React.MouseEvent
  ) => {
    ...
    console.log(
      "[CheckpointsPane] Merge clicked for checkpoint:",
    ...
    try {
      ...
      console.log(
        `[CheckpointsPane] Step 1: Reverting to checkpoint ${commitHash.slice(0, 7)}`
      );
      ...
      console.log(
        `[CheckpointsPane] Successfully reverted to checkpoint ${commitHash.slice(0, 7)}`
      );
      ...
      setTimeout(() => {
        ...
        if (!mergeButton) {
          toast.error(
            "Merge control not available. Please refresh or ensure session is ready."
          );
          return;
        }
        mergeButton.click();
  ...
  const openFileDiff = async (
    ...
    try {
      ...
      if (!haveEither) {
        ...
        const match =
          parsed.files.find((f) => f.fileName === normalized) ||
          parsed.files.find(
            (f) =>
              f.fileName &&
              (f.fileName.endsWith("/" + normalized) ||
                normalized.endsWith(f.fileName))
          );
        if (!match) {
      ...
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-white/30 text-sm">
            Checkpoints will appear as the agent makes progress
          </p>
        </div>
      </div>
    );
  ...
  return (
    <>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {checkpoint.commit_hash &&
                        firstCheckpointHash &&
                        checkpoint.commit_hash !== firstCheckpointHash && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleMergeCheckpoint(checkpoint.commit_hash, e);
                            }}
                    ...
                    {checkpointFileChanges.map((file, index) => {
                      ...
                      return (
                        <motion.button
                          key={file.path}
                          onClick={() =>
                            toHash && openFileDiff(file.path, fromHash, toHash)
                          }
                ...
                <div className="h-full flex flex-col p-6">
                  ...
                  >
                    ...
                    <p className="text-sm text-white/50">
                      Total changes across {checkpoints.length} checkpoint
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/50">
                        {checkpoints.length} checkpoints • Review changes before
                        merging
                      </p>
                    </div>
                  </div>
      ...
      {diffViewer.isOpen &&
        diffViewer.originalContent !== undefined &&
        diffViewer.modifiedContent !== undefined && (
          <DiffViewer
