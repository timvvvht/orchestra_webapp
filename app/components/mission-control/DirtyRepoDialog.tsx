...
import { useGitStage, useGitCommit, useGitStash, useGitDiscard } from '../../hooks/useGitMutations';
...
const GitFileList: React.FC<GitFileListProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status.trim()) {
      case 'M': return 'text-blue-400'; // Modified
      case 'A': return 'text-green-400'; // Added
      case 'D': return 'text-red-400'; // Deleted
      case 'R': return 'text-purple-400'; // Renamed
      case '??': return 'text-yellow-400'; // Untracked
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-medium text-white/90">Changed Files ({stageableFiles.length})</h3>
          <div className="flex gap-2">
            <Button size="sm" onClick={onSelectAll}>Select All</Button>
            <Button size="sm" onClick={onSelectNone}>Select None</Button>
          </div>
        </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className={`text-xs font-mono px-1.5 py-0.5 rounded ${getStatusColor(file.status)}`}>
                  {file.status}
                </span>
                <span className="text-sm font-medium truncate text-white/90">{file.path}</span>
              </div>
...
export default function DirtyRepoDialog({
  ...
  // Hooks
  const { data: gitStatus, refetch: refetchStatus } = useGitStatus(projectRoot, open);
  const { data: diffContent } = useGitDiffFile(projectRoot, currentFile || '', !!currentFile);
  ...
  const handleStageAndCommit = useCallback(async () => {
    ...
    try {
      ...
      const res: StageResult = await stageMutation.mutateAsync({ projectRoot, files: selectedFiles });
  ...
  const handleCommit = useCallback(async () => {
    ...
    try {
      await commitMutation.mutateAsync({ projectRoot, message: commitMessage.trim() });
  ...
  const isLoading = stageMutation.isPending || commitMutation.isPending || 
                   stashMutation.isPending || discardMutation.isPending;
          ...
          {currentFile ? (
            <>
              <div className="p-4 border-b border-white/10">
                ...
                <div className="text-sm text-white/60">
                  {files.find(f => f.path === currentFile)?.status} - Click to view diff
                </div>
              </div>
            ...
            <div className="flex-1 flex items-center justify-center text-white/50">
              <div className="text-center">
                <div className="text-lg mb-2">Select a file to view changes</div>
                <div className="text-sm">Click on a file in the left panel to see its diff</div>
              </div>
            </div>
          )}
        </div>
