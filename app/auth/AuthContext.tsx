...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const exchangeSupabaseSessionForACSCookies = useCallback(async (session: any) => {
    try {
      console.log('[Auth] Attempting ACS cookie exchange with session for user:', session.user?.email);
      const acsClient = getDefaultACSClient();
      const response = await acsClient.getClient().exchangeTokenWithACS(session);
  ...
  useEffect(() => {
    ...
    const seedToken = async () => {
      ...
      try {
        ...
        if (data.session) {
          ...
          if (accessToken) {
            ...
            if (current !== accessToken) {
              ...
              console.log('[DEBUG notifyAuthChange] authenticated', data.session.user.id);
          ...
          console.log('[DEBUG notifyAuthChange] seeding with single-player-user');
          console.log('🔌 [AuthContext] Starting SSE relay for single-player mode');
        ...
        console.log('[DEBUG notifyAuthChange] seeding with single-player-user (error fallback)');
        console.log('🔌 [AuthContext] Starting SSE relay for single-player mode (error fallback)');
        notifyAuthChange('single-player-user');
      }
    };
    seedToken();
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (evt, session) => {
      try {
        ...
        console.log(`🔔 [AuthContext] [${timestamp}] *** SUPABASE AUTH EVENT: ${evt} ***`);
        ...
        console.log('🔍 [AuthContext] Current user state before processing:', {
          currentUser: user?.email || 'null',
          currentUserId: user?.id || 'null'
        });

        if (evt === 'SIGNED_IN' || evt === 'INITIAL_SESSION' || evt === 'TOKEN_REFRESHED') {
          console.log(`🔄 [AuthContext] Processing ${evt} event - setting user`);
          ...
          if (session) { // Condition made more robust; getSupabaseAccessToken handles null session
            ...
            if (accessToken) {
              ...
              if (current !== accessToken) {
                ...
                // 🔌 Start SSE relay for authenticated user
                console.log('[DEBUG notifyAuthChange] authenticated', session.user.id);
                notifyAuthChange(session.user.id);
              } else {
                // token unchanged – skip redundant processing
                console.log('[AuthContext] access_token unchanged, skipping setAuthToken');
              }
            } else {
              // Helper returned null token - clear ACS auth
              console.warn('⚠️ [AuthContext] No valid token found after filtering, clearing ACS auth');
              acsClient.clearAuthToken();
            }
            ...
            if (accessToken && accessToken === lastExchangedTokenRef.current) {
              console.log('[Auth] Access token unchanged or already exchanged – skip ACS exchange');
            } else if (accessToken && exchangeInFlightRef.current) {
              console.log('[Auth] Exchange already in flight – skip');
            } else if (accessToken) { // Only proceed if accessToken is valid and not caught by above guards
              console.log('[Auth] Attempting ACS cookie exchange with session for user (debounced):', session.user?.email);
        ...
        if (evt === 'SIGNED_OUT') {
          ...
          console.log('[DEBUG notifyAuthChange] sign-out fallback single-player');
          console.log('🔌 [AuthContext] Starting SSE relay for single-player mode after sign out');
          notifyAuthChange('single-player-user');
        }
