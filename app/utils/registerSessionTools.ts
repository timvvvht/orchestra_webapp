...
export async function registerSessionTools({
  ...
  try {
    ...
    console.log(
      `🔧 [registerSessionTools] Registering ${tools.length} tools for session ${sessionId.slice(0, 8)}: ${toolNames}`
    );
    ...
    console.log(
      `✅ [registerSessionTools] Successfully registered ${tools.length} tools: ${toolNames}`
    );
  } catch (error) {
...
export function createCatToolSpec(): ToolSpec {
  return {
    name: "cat",
    description:
      "Outputs the contents of a file, with optional line range support.",
    input_schema: {
      type: "object",
      properties: {
        file: {
          type: "string",
          description:
            "File path to read, relative to the current directory if not absolute.",
        },
        view_range: {
          type: "object",
    ...
    input_schema: {
      ...
      properties: {
        path: {
          type: "string",
          description:
            'Directory to visualize. Can be relative or absolute. Defaults to current directory (".").',
          default: ".",
        },
        depth: {
          type: "integer",
          description:
            "Optional depth limit for the directory tree (max 3). For example, 3 limits the tree to 3 levels.",
          maximum: 3,
          default: 3,
        },
        include_files: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        agent_config: {
          type: "string",
          description:
            'Optional agent configuration name to use (defaults to "general")',
          default: "general",
        },
        timeout_sec: {
          type: "integer",
          description:
            "Optional timeout in seconds for the search operation (defaults to 120)",
          default: 120,
        },
      },
      required: ["prompt"],
    ...
    input_schema: {
      ...
      properties: {
        files: {
          ...
          description:
            "Array of file paths to read (maximum 25 files per request)",
        },
      },
      required: ["files"],
    },
...
export function createSearchFilesToolSpec(): ToolSpec {
  return {
    name: "search_files",
    description:
      "Search for files by name and content using ripgrep. Supports filename patterns, content search, case-insensitive search, and pagination.",
    input_schema: {
      type: "object",
      properties: {
        root: {
          type: "string",
          description:
            "Directory root to search in (defaults to current directory)",
          default: ".",
        },
        filename_glob: {
          type: "string",
        ...
        ignore_case: {
          type: "boolean",
          description:
            "Case-insensitive search (applies to both path & content)",
          default: false,
        },
        context_lines: {
          type: "integer",
...
export function createStrReplaceEditorToolSpec(): ToolSpec {
  return {
    ...
    description:
      "A versatile file editing tool that supports viewing, creating, appending, inserting, and string replacement operations on files.",
    input_schema: {
      type: "object",
      properties: {
        command: {
          type: "string",
          description:
            "Command to execute: 'view', 'create', 'str_replace', 'insert', 'append'",
        ...
        view_range: {
          ...
          description:
            "Optional range of lines to display [start, end] (1-based indexing)",
          minItems: 2,
          maxItems: 2,
        },
        file_text: {
        ...
        occurrence_index: {
          type: "integer",
          description:
            "0-based index of which occurrence to replace (for multiple matches)",
          minimum: 0,
        },
        insert_line: {
          type: "integer",
    ...
    input_schema: {
      ...
      properties: {
        path: {
          type: "string",
          description:
            'Directory path to list contents of. Defaults to current directory (".")',
          default: ".",
        },
        include_hidden: {
          type: "boolean",
          description:
            "Whether to include hidden files/directories (starting with dot). Defaults to false.",
          default: false,
        },
        only_dirs: {
          type: "boolean",
          description:
            "Whether to only include directories in the output. Defaults to false.",
          default: false,
        },
      },
      required: [],
...
export function createMyNewToolSpec(): ToolSpec {
  return {
    name: "my_new_tool",
    description:
      "An example tool that processes a message with an optional count parameter, demonstrating the custom tool development pattern.",
    input_schema: {
      type: "object",
      properties: {
        message: {
...
export function createPathSecurityToolSpec(): ToolSpec {
  return {
    name: "path_security",
    description:
      "Validates that a given path is inside the current working directory, preventing access to files outside the intended scope.",
    input_schema: {
      type: "object",
      properties: {
        path: {
...
export function createAwsToolsSpec(): ToolSpec {
  return {
    name: "aws_tools",
    description:
      "Search and explore AWS-related tools and functions. Supports searching AWS service definitions, functions, and classes.",
    input_schema: {
      type: "object",
      properties: {
        service: {
        ...
        root: {
          type: "string",
          description:
            "Root directory to search for AWS tools (defaults to tools directory)",
          default: "/Users/tim/Code/computer-use/tools",
        },
        filename_glob: {
          type: "string",
          description:
            "Filename pattern to filter AWS tool files (default: *aws*.py)",
          default: "*aws*.py",
        },
      },
      required: [],
    ...
    input_schema: {
      ...
      properties: {
        session_id_prefix: {
          type: "string",
          description:
            "Prefix for the generated session ID. Defaults to 'shell_session'.",
        },
        initial_cwd: {
          type: ["string", "null"],
        ...
        initial_env_vars: {
          type: ["object", "null"],
          description:
            "Optional map of env var name → value to preload into the session.",
          additionalProperties: { type: "string" },
        },
        prefer_login_shell: {
          type: "boolean",
          description:
            "If true, start the shell as a login shell (e.g., bash -l).",
          default: false,
        },
      },
      required: [],
    ...
    input_schema: {
      ...
      properties: {
        ...
        timeout_seconds: {
          ...
          minimum: 1,
          description:
            "Maximum time to allow the command to run before killing it. Defaults to 120 seconds.",
          default: 120,
        },
      },
  ...
  return {
    ...
    input_schema: {
      ...
      properties: {
        ...
        path: {
          type: "string",
          description: "Absolute or relative path to new cwd.",
        },
      },
...
export function createGetRunnerSessionStateSpec(): ToolSpec {
  return {
    ...
    description:
      "Retrieve current cwd, environment variables, and active background jobs for a session.",
    input_schema: {
      type: "object",
      properties: {
        session_id: { type: "string" },
...
export function createStartBackgroundOsJobInSessionSpec(): ToolSpec {
  return {
    ...
    description:
      "Start a long-running background OS process inside a session. Returns an internal job ID and OS PID.",
    input_schema: {
      type: "object",
      properties: {
        session_id: { type: "string" },
        command_parts: {
          ...
          items: { type: "string" },
          description:
            "Command broken into argv parts – first element is executable, rest are args.",
        },
      },
...
export function createGetBackgroundOsJobStatusSpec(): ToolSpec {
  return {
    ...
    description:
      "Query status of a background OS job previously started in the session.",
    input_schema: {
      type: "object",
      properties: {
        session_id: { type: "string" },
...
export function createSendSignalToOsJobSpec(): ToolSpec {
  return {
    ...
    description:
      "Send a POSIX signal (e.g., SIGTERM) to a running background OS job.",
    input_schema: {
      type: "object",
      properties: {
        session_id: { type: "string" },
 ...
 */
export async function registerApplyPatchTool(
  sessionId: string,
  ...
  console.log(
    "🔧 [registerApplyPatchTool] ✅ apply_patch tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
...
export async function registerCatTool(
  ...
  console.log(
    "🔧 [registerCatTool] ✅ cat tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerTreeTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  ...
  console.log(
    "🔧 [registerTreeTool] ✅ tree tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerAgenticSearchBackgroundTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  console.log(
    "🔧 [registerAgenticSearchBackgroundTool] 📋 Registering agentic_search_background tool:",
    {
      sessionId: sessionId.slice(0, 8) + "...",
      baseUrl: options.baseUrl || "default",
      hasAuthToken: !!options.authToken,
  ...
  console.log(
    "🔧 [registerAgenticSearchBackgroundTool] 📋 Tool spec created:",
    {
 ...
 */
export async function registerSearchNotesTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  console.log(
    "🔧 [registerSearchNotesTool] 📋 Registering search_notes tool:",
    {
      sessionId: sessionId.slice(0, 8) + "...",
      baseUrl: options.baseUrl || "default",
      hasAuthToken: !!options.authToken,
  ...
  console.log(
    "🔧 [registerSearchNotesTool] ✅ search_notes tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerReadFilesTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  ...
  console.log(
    "🔧 [registerReadFilesTool] ✅ read_files tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerSearchFilesTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  console.log(
    "🔧 [registerSearchFilesTool] 📋 Registering search_files tool:",
    {
      sessionId: sessionId.slice(0, 8) + "...",
      baseUrl: options.baseUrl || "default",
      hasAuthToken: !!options.authToken,
  ...
  console.log(
    "🔧 [registerSearchFilesTool] ✅ search_files tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerStrReplaceEditorTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  console.log(
    "🔧 [registerStrReplaceEditorTool] 📋 Registering str_replace_editor tool:",
    {
      sessionId: sessionId.slice(0, 8) + "...",
      baseUrl: options.baseUrl || "default",
      hasAuthToken: !!options.authToken,
  ...
  console.log(
    "🔧 [registerStrReplaceEditorTool] ✅ str_replace_editor tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerMyNewToolTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  ...
  console.log(
    "🔧 [registerMyNewToolTool] ✅ my_new_tool tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerPathSecurityTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  console.log(
    "🔧 [registerPathSecurityTool] 📋 Registering path_security tool:",
    {
      sessionId: sessionId.slice(0, 8) + "...",
      baseUrl: options.baseUrl || "default",
      hasAuthToken: !!options.authToken,
  ...
  console.log(
    "🔧 [registerPathSecurityTool] ✅ path_security tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
 ...
 */
export async function registerAwsToolsTool(
  sessionId: string,
  options: { baseUrl?: string; authToken?: string } = {}
): Promise<void> {
  ...
  console.log(
    "🔧 [registerAwsToolsTool] ✅ aws_tools tool registration completed for session:",
    sessionId.slice(0, 8) + "..."
  );
}
