 ...
 * @returns Promise<boolean> - true if repo is dirty, false if clean
 */
export async function checkRepoDirtyLightweight(projectRoot: string): Promise<boolean> {
    console.log(`[worktreeApi][checkRepoDirtyLightweight] Checking dirty status for: ${projectRoot}`);
 ...
 */
export async function autoCommitRepo(projectRoot: string, message: string): Promise<void> {
    try {
 ...
 */
export async function invokeCreateWorktree(sessionId: string, projectRoot: string): Promise<CreateWorktreeResponse> {
    try {
        console.log(`🔥🔥🔥 [DEBUG] >>>>>>>>>>>>>>>>> invokeCreateWorktree CALLED with sessionId=${sessionId.slice(0, 8)}..., projectRoot=${projectRoot}`);
        console.log(`🌳 [worktreeApi] Creating worktree for session: ${sessionId.slice(0, 8)}... in project: ${projectRoot}`);

        // Check if repository is dirty before creating worktree
        console.log(`🔍 [worktreeApi] Checking repository dirty status before creating worktree...`);
        const isDirty = await checkRepoDirtyRaw(projectRoot);

        if (isDirty) {
            console.warn(`🚨 [worktreeApi] Repository is dirty - this will need to be handled by the UI`);
        ...
        console.log(`✅ [worktreeApi] Repository is clean, proceeding with worktree creation`);
        console.log(`🔥 [DEBUG] About to call Tauri invoke('create_worktree') with params:`, {
            sessionId: sessionId,
            projectRoot: projectRoot
        });
        ...
        console.error(`🔥🔥🔥 [DEBUG] >>>>>>>>>>>>>>>>> invokeCreateWorktree FAILED with error:`, error);
        console.error(`❌ [worktreeApi] Failed to create worktree for session ${sessionId}:`, error);
 ...
 */
export async function invokeMergeWorktree(sessionId: string, projectRoot: string): Promise<FinalizeResultResponse> {
    try {
        ...
        console.error(`❌ [worktreeApi] Failed to merge-only for session ${sessionId}:`, error);
 ...
 * @returns Promise with merge result
 */
export async function invokeFinalizeWorktree(sessionId: string, projectRoot: string, keepWorktree?: boolean): Promise<FinalizeResultResponse> {
    try {
        ...
        console.error(`❌ [worktreeApi] Failed to finalize worktree for session ${sessionId}:`, error);
 ...
 * @returns Promise with workspace path and metadata
 */
export async function ensureWorktreeForSession(sessionId: string, codePath: string): Promise<CreateWorktreeResponse> {
    try {
        console.log(`🔧 [worktreeApi] Ensuring worktree exists for session: ${sessionId.slice(0, 8)}... in project: ${codePath}`);
        ...
        console.log(`✅ [worktreeApi] Worktree ensured for session: ${sessionId.slice(0, 8)}...`);
        return result;
    } catch (error) {
        console.error(`❌ [worktreeApi] Failed to ensure worktree for session ${sessionId}:`, error);
...
export async function createWorktreeWithDirtyCheck(
    sessionId: string,
    projectRoot: string,
    onShowDirtyModal?: (projectRoot: string, onCommitAndContinue: () => Promise<void>) => void
    ...
    try {
        ...
        if (error instanceof Error && error.message === 'DIRTY_REPO') {
            ...
            if (!onShowDirtyModal) {
                ...
                throw new Error('Repository has uncommitted changes. Please commit or stash your changes before creating a worktree.');
            }
