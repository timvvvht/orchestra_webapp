import { type PlanProgress, type PlanItem } from "@/types/plans";
 ...
 */
export function analyzePlanProgress(
  markdownContent: string
): SimplePlanProgress {
  console.log(
    `[analyzePlanProgress] Analyzing markdown content (length: ${markdownContent.length} chars)`
  );
  ...
  const checkedPattern =
    /^(\s*)-\s*\[x\]\s*(.+?)(?:\s*<!--\s*id:([a-f0-9]+)\s*-->.*)?$/i;
  const uncheckedPattern =
    /^(\s*)-\s*\[\s*\]\s*(.+?)(?:\s*<!--\s*id:([a-f0-9]+)\s*-->.*)?$/i;
  ...
  const percentage =
    total > 0 ? Math.round((completed / total) * 100 * 100) / 100 : 0; // Round to 2 decimal places
  ...
  console.log(
    `[analyzePlanProgress] Analysis complete: ${total} total items, ${completed} completed, ${percentage}% complete`
  );
  return result;
}
 ...
 */
export function analyzePlanProgressDetailed(
  markdownContent: string
): PlanProgress {
  ...
  console.log(
    `[PlanProgress] START analysis - length: ${markdownContent.length} chars, hash: ${contentHash}`
  );

  // Show first few lines for context
  const previewLines = markdownContent.split("\n").slice(0, 3);
  console.log(
    `[PlanProgress] Content preview:`,
    previewLines.map((line) => `"${line.substring(0, 80)}"`)
  );
  ...
  console.log(
    `[PlanProgress] Scanning ${lines.length} lines for checkbox items...`
  );
  ...
  lines.forEach((line, index) => {
    ...
    if (checkedMatch) {
      ...
      items.push(item);
      console.log(
        `[PlanProgress] Line ${lineNumber}: CHECKED - id:${todoId || "none"} text:"${cleanText.substring(0, 50)}${cleanText.length > 50 ? "..." : ""}"`
      );
      return;
    }
    ...
    if (uncheckedMatch) {
      ...
      items.push(item);
      console.log(
        `[PlanProgress] Line ${lineNumber}: UNCHECKED - id:${todoId || "none"} text:"${cleanText.substring(0, 50)}${cleanText.length > 50 ? "..." : ""}"`
      );
    }
  });
  ...
  const duplicateIds = idsWithValues.filter(
    (id, index) => idsWithValues.indexOf(id) !== index
  );
  if (duplicateIds.length > 0) {
    console.warn(`[PlanProgress] DUPLICATE todo IDs detected:`, [
  ...
  const percent =
    total > 0 ? Math.round((checked / total) * 100 * 100) / 100 : 0; // Round to 2 decimal places

  console.log(
    `[PlanProgress] METRICS - total: ${total}, checked: ${checked}, unchecked: ${unchecked}, percent: ${percent}%`
  );
  ...
  checkedItems.forEach((item) => {
    console.log(
      `  ✓ Line ${item.line_number}: ${item.text.substring(0, 60)}${item.text.length > 60 ? "..." : ""} ${item.id ? `(id:${item.id})` : ""}`
    );
  });
