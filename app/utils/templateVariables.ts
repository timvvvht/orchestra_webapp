 ...
 */
export async function createACSTemplateVariables(context?: {
  projectName?: string;
  sessionId?: string;
  agentName?: string;
}): Promise<CoreTemplateVars> {
  ...
  if (username === "User") {
    ...
    if (username === "User") {
      console.warn(
        'No user name found in authService.user.user_metadata.name or getUserName(), using "User"'
      );
    }
  }
