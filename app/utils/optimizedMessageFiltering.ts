...
import { getFileOperationsForResponse, getToolCallsForResponse } from '@/utils/conversationBoundaries';
 ...
 */
function computeFinalMessageFlags(messages: ChatMessage[], messageIndices: Map<string, number>): Map<string, boolean> {
 ...
 */
export function getOptimizedVisibleMessages(messages: ChatMessage[]): ChatMessage[] {
 ...
 */
export function isOptimizedFinalAssistantMessage(message: ChatMessage, allMessages: ChatMessage[]): boolean {
 ...
 */
export function getOptimizedFileOperationsForResponse(messages: ChatMessage[], finalMessageId: string): any[] {
    const messagesHash = createMessagesHash(messages);
 ...
 */
export function getOptimizedToolCallsForResponse(messages: ChatMessage[], finalMessageId: string): any[] {
    const messagesHash = createMessagesHash(messages);
...
export function getMessageFilterCacheStats(): {
    ...
    return {
        ...
        operationsCacheSize: operationsCache ? operationsCache.fileOperations.size + operationsCache.toolCalls.size : 0
    };
}
