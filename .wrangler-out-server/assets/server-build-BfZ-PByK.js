import { r as requireReact, a as requireReactDom, S as ServerRouter, w as withComponentProps, b as withErrorBoundaryProps, M as Meta, L as Links, c as ScrollRestoration, d as Scripts, O as Outlet, i as isRouteErrorResponse, u as useActionData, e as useLoaderData, f as reactExports, F as Form } from "./app-DDgCEnuo.js";
...
function requireReactDomServer_edge_production() {
  ...
  function murmurhash3_32_gc(key2, seed) {
    var remainder = key2.length & 3;
    var bytes = key2.length - remainder;
    var h1 = seed;
    for (seed = 0; seed < bytes; ) {
      var k1 = key2.charCodeAt(seed) & 255 | (key2.charCodeAt(++seed) & 255) << 8 | (key2.charCodeAt(++seed) & 255) << 16 | (key2.charCodeAt(++seed) & 255) << 24;
      ++seed;
    ...
    switch (remainder) {
      case 3:
        k1 ^= (key2.charCodeAt(seed + 2) & 255) << 16;
      case 2:
        k1 ^= (key2.charCodeAt(seed + 1) & 255) << 8;
      case 1:
        k1 ^= key2.charCodeAt(seed) & 255, k1 = 3432918353 * (k1 & 65535) + ((3432918353 * (k1 >>> 16) & 65535) << 16) & 4294967295, k1 = k1 << 15 | k1 >>> 17, h1 ^= 461845907 * (k1 & 65535) + ((461845907 * (k1 >>> 16) & 65535) << 16) & 4294967295;
    }
    h1 ^= key2.length;
    h1 ^= h1 >>> 16;
  ...
  function pushAdditionalFormField(value, key2) {
    this.push(startHiddenInputChunk);
    validateAdditionalFormField(value);
    pushStringAttribute(this, "name", key2);
    ...
    switch (type) {
      ...
      case "script":
        ...
        if ("string" !== typeof props.src || !props.src || !asyncProp || "function" === typeof asyncProp || "symbol" === typeof asyncProp || props.onLoad || props.onError || 4 === formatContext.insertionMode || formatContext.tagScope & 1 || null != props.itemProp)
          ...
          var resourceState$jscomp$0 = resources.hasOwnProperty(key2) ? resources[key2] : void 0;
          if (null !== resourceState$jscomp$0) {
            resources[key2] = null;
            var scriptProps = props;
            if (resourceState$jscomp$0) {
              2 === resourceState$jscomp$0.length && (scriptProps = assign({}, props), adoptPreloadCredentials(scriptProps, resourceState$jscomp$0));
              var preloadResource$jscomp$0 = preloads.get(key2);
      ...
      if (as && href) {
        switch (as) {
          case "image":
            ...
            var key2 = imageSrcSet ? imageSrcSet + "\n" + (imageSizes || "") : href;
            if (resumableState.imageResources.hasOwnProperty(key2)) return;
            resumableState.imageResources[key2] = PRELOAD_NO_CREDS;
            resumableState = renderState.headers;
            var header;
            resumableState && 0 < resumableState.remainingCapacity && "string" !== typeof imageSrcSet && "high" === fetchPriority && (header = getPreloadAsHeader(href, as, options), 0 <= (resumableState.remainingCapacity -= header.length + 2)) ? (renderState.resets.image[key2] = PRELOAD_NO_CREDS, resumableState.highImagePreloads && (resumableState.highImagePreloads += ", "), resumableState.highImagePreloads += header) : (resumableState = [], pushLinkImpl(
              resumableState,
              assign(
                { rel: "preload", href: imageSrcSet ? void 0 : href, as },
                options
              )
            ), "high" === fetchPriority ? renderState.highImagePreloads.add(resumableState) : (renderState.bulkPreloads.add(resumableState), renderState.preloads.images.set(key2, resumableState)));
            break;
          case "style":
          ...
          default:
            ...
            if ((resumableState = renderState.headers) && 0 < resumableState.remainingCapacity && "font" === as && (key2 = getPreloadAsHeader(href, as, options), 0 <= (resumableState.remainingCapacity -= key2.length + 2)))
              renderState.resets.font[href] = PRELOAD_NO_CREDS, resumableState.fontPreloads && (resumableState.fontPreloads += ", "), resumableState.fontPreloads += key2;
            else
  ...
  function retryNode(request, task) {
    ...
    if (null !== node) {
      if ("object" === typeof node) {
        switch (node.$$typeof) {
          case REACT_ELEMENT_TYPE:
            var type = node.type, key2 = node.key, props = node.props;
            node = props.ref;
            var ref = void 0 !== node ? node : null, name = getComponentNameFromType(type), keyOrIndex = null == key2 ? -1 === childIndex ? 0 : childIndex : key2;
            key2 = [task.keyPath, name, keyOrIndex];
            if (null !== task.replay)
              a: {
                ...
                for (node = 0; node < childIndex.length; node++) {
                  ...
                  if (keyOrIndex === node$jscomp$0[1]) {
                    if (4 === node$jscomp$0.length) {
                      ...
                      try {
                        renderElement(request, task, key2, type, props, ref);
                      ...
                      b: {
                        ...
                        task = createReplayTask(
                          ...
                          props.fallbackState,
                          fallbackAbortSet,
                          [key2[0], "Suspense Fallback", key2[2]],
                          task.formatContext,
                          task.context,
                          task.treeContext,
                          task.componentStack,
            ...
            else renderElement(request, task, key2, type, props, ref);
            return;
          case REACT_PORTAL_TYPE:
            throw Error(
          ...
          if (!shellComplete) {
            ...
            b: for (; 0 < headers.remainingCapacity && !queueStep.done; queueStep = queueIter.next())
              for (var sheetIter = queueStep.value.sheets.values(), sheetStep = sheetIter.next(); 0 < headers.remainingCapacity && !sheetStep.done; sheetStep = sheetIter.next()) {
                var sheet = sheetStep.value, props = sheet.props, key2 = props.href, props$jscomp$0 = sheet.props, header = getPreloadAsHeader(props$jscomp$0.href, "style", {
                ...
                if (0 <= (headers.remainingCapacity -= header.length + 2))
                  renderState.resets.style[key2] = PRELOAD_NO_CREDS, linkHeader && (linkHeader += ", "), linkHeader += header, renderState.resets.style[key2] = "string" === typeof props.crossOrigin || "string" === typeof props.integrity ? [props.crossOrigin, props.integrity] : PRELOAD_NO_CREDS;
                else break b;
              }
          }
...
function Glass(props) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ...rest, className: `glass-panel ${className}` });
}
function Input({ className = "", variant = "default", ...rest }) {
  const variantCls = variant === "code" ? " font-mono tracking-wider" : "";
  return /* @__PURE__ */ jsxRuntimeExports.jsx("input", { ...rest, className: `input${variantCls} ${className}` });
}
function Button({ className = "", children, ...rest }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { ...rest, className: `btn-primary ${className}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute inset-0 bg-gradient-to-r from-blue-400/20 to-purple-400/20 rounded-xl opacity-0 hover:opacity-100 transition-opacity" }),
...
function rlKey(ip, windowSec) {
  const now = Math.floor(Date.now() / 1e3);
  const bucket = Math.floor(now / windowSec);
  return `rl:${ip}:${bucket}`;
}
async function checkRateLimit(env, ip) {
  const windowSec = Number(env.RATE_LIMIT_WINDOW_SEC ?? 600);
  const max = Number(env.RATE_LIMIT_MAX_ATTEMPTS ?? 5);
  const key2 = rlKey(ip, windowSec);
  const currentRaw = await env.DOWNLOAD_TOKENS.get(key2);
  let count = currentRaw ? parseInt(currentRaw, 10) : 0;
  count++;
  await env.DOWNLOAD_TOKENS.put(key2, String(count), { expirationTtl: windowSec });
  return { allowed: count <= max, remaining: Math.max(0, max - count) };
}
function key(token) {
  return `dl:${token}`;
}
async function issueToken(env, ip, ttlSec) {
  const token = crypto.randomUUID();
  const now = Date.now();
  const data = { ip, created: now, expires: now + ttlSec * 1e3 };
...
async function action({
  ...
  const {
    allowed
  } = await checkRateLimit(context.cloudflare.env, ip);
  if (!allowed || !code) return Response.json({
    error: "Invalid code"
  }, {
    status: 401
  });
  ...
  const token = await issueToken(context.cloudflare.env, ip, 600);
  return Response.redirect(`/download?token=${encodeURIComponent(token)}`);
}
const landing = withComponentProps(function Landing({}) {
  const data = useActionData();
  const info = useLoaderData();
  const [isAppleSilicon, setIsAppleSilicon] = reactExports.useState(null);
  reactExports.useEffect(() => {
    const anyNav = navigator;
    if (anyNav?.userAgentData?.getHighEntropyValues) {
      anyNav.userAgentData.getHighEntropyValues(["architecture"]).then((res) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", {
    className: "min-h-screen relative orchestra-page",
    children: [/* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
      className: "page-void",
      children: [/* @__PURE__ */ jsxRuntimeExports.jsx("div", {
        className: "page-void-gradient"
      }), /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
        className: "page-void-orb-blue"
      }), /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
        className: "page-void-orb-purple"
      })]
    }), /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
      className: "relative z-10 flex items-center justify-center py-24 px-6",
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Glass, {
        className: "content-narrow w-full p-10",
        children: [/* @__PURE__ */ jsxRuntimeExports.jsx("div", {
          className: "absolute inset-0 bg-gradient-to-br from-white/[0.01] to-transparent pointer-events-none"
        }), /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
          className: "gradient-frame",
          children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
            className: "frame-content p-8 space-y-4",
            children: [/* @__PURE__ */ jsxRuntimeExports.jsx("h1", {
              className: "text-display text-white/90",
              children: "Orchestra"
            }), /* @__PURE__ */ jsxRuntimeExports.jsx("p", {
              className: "text-body text-white/70",
              children: "The Sane Way to Vibe Code."
            }), info.isMac && isAppleSilicon === true && /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
              className: "text-xs-ox text-white/50",
              children: "Apple Silicon detected"
            }), info.isMac && isAppleSilicon === false && /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
              className: "text-xs-ox text-white/50",
              children: "Intel Mac detected"
              ...
              children: [/* @__PURE__ */ jsxRuntimeExports.jsx("label", {
                ...
                inputMode: "text",
                spellCheck: false
              }), data?.error && /* @__PURE__ */ jsxRuntimeExports.jsx("p", {
                ...
                className: "text-micro text-white/40",
...
const serverManifest = { "entry": { "module": "/assets/entry.client-D2ZbjaWZ.js", "imports": ["/assets/chunk-ZYFC6VSF-DjZdqd-6.js"], "css": [] }, "routes": { "root": { "id": "root", "parentId": void 0, "path": "", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": true, "module": "/assets/root-pg-qvyq5.js", "imports": ["/assets/chunk-ZYFC6VSF-DjZdqd-6.js"], "css": ["/assets/root-LeHXVKkQ.css"], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 }, "routes/landing": { "id": "routes/landing", "parentId": "root", "path": void 0, "index": true, "caseSensitive": void 0, "hasAction": true, "hasLoader": true, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": false, "module": "/assets/landing-DvhWXk0x.js", "imports": ["/assets/chunk-ZYFC6VSF-DjZdqd-6.js"], "css": [], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 } }, "url": "/assets/manifest-c2e073ae.js", "version": "c2e073ae", "sri": void 0 };
